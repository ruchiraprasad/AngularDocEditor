// import { Component, OnInit } from '@angular/core';

// @Component({
//   selector: 'app-editor',
//   templateUrl: './editor.component.html',
//   styleUrls: ['./editor.component.css']
// })
// export class EditorComponent implements OnInit {

//   constructor() { }

//   ngOnInit() {
//   }

// }





import { Component, ViewEncapsulation, ViewChild, Input } from '@angular/core';
// tslint:disable-next-line:max-line-length
import {
    RequestNavigateEventArgs, ViewChangeEventArgs, DocumentEditorComponent,
    PrintService, SfdtExportService, WordExportService, TextExportService, SelectionService,
    SearchService, EditorService, ImageResizerService, EditorHistoryService, ContextMenuService,
    OptionsPaneService, HyperlinkDialogService, TableDialogService, BookmarkDialogService, TableOfContentsDialogService,
    PageSetupDialogService, StyleDialogService, ListDialogService, ParagraphDialogService, BulletsAndNumberingDialogService,
    FontDialogService, TablePropertiesDialogService, BordersAndShadingDialogService, TableOptionsDialogService,
    CellOptionsDialogService, StylesDialogService, ContextType, HighlightColor, BorderSettings, BorderType, LineStyle, DocumentEditor, TableOfContentsSettings
} from '@syncfusion/ej2-ng-documenteditor';
import { ClickEventArgs, ToolbarComponent, TabComponent } from '@syncfusion/ej2-ng-navigations';
import { Dialog } from '@syncfusion/ej2-ng-popups';
import { isNullOrUndefined, createElement, KeyboardEventArgs } from '@syncfusion/ej2-base';
import { Button } from '@syncfusion/ej2-ng-buttons';
import { ColorPickerEventArgs } from '@syncfusion/ej2-ng-inputs';
import { TitleBar } from './titlebar';
import { StatusBar } from './statusbar';
import { DocumentLoader } from './document-loader';
import { TemplateLoader } from './template-loader';
import { DropDownList } from '@syncfusion/ej2-ng-dropdowns';
import { NumericTextBox } from '@syncfusion/ej2-ng-inputs';
import { CheckBox } from '@syncfusion/ej2-ng-buttons';
import { SplitButton } from '@syncfusion/ej2-splitbuttons';
import { DropDownButton } from '@syncfusion/ej2-ng-splitbuttons';
import { Query } from '@syncfusion/ej2-data';

import { map } from 'rxjs/operators';
import { HttpClient, HttpParams } from '@angular/common/http';
import { TemplateListItem } from './models/templateListItem';
import { DocumentEditorService } from './shared/editor.service';



import { Template } from '../shared/template.model';
import { FieldCategory, Field } from './models/field';
/**
 * Document Editor Component
 */
@Component({
    selector: 'app-editor',
    templateUrl: './editor.component.html',
    styleUrls: ['./editor.component.css'],
    encapsulation: ViewEncapsulation.None,

})
export class EditorComponent {

    @ViewChild('documenteditor_default')
    public documentEditor: DocumentEditorComponent;
    @ViewChild('style_dropdown')
    public styleDropDown: DropDownList;
    @ViewChild('table_style_dropdown')
    public tableStyleDropDown: DropDownList;
    @ViewChild('highlight_color')
    public highlightColor: SplitButton;
    @ViewChild('table_highlight_color')
    public tableHighlightColor: SplitButton;
    @ViewChild('tableAll_border')
    tableAllBorder: Button;
    @ViewChild('outline_border')
    outlineBorder: Button;
    @ViewChild('tableCenter_border')
    tableCenterBorder: Button;
    @ViewChild('tableLeft_border')
    tableLeftBorder: Button;
    @ViewChild('tableCenterV_border')
    tableCenterVBorder: Button;
    @ViewChild('tableRight_border')
    tableRightBorder: Button;
    @ViewChild('tableTop_border')
    tableTopBorder: Button;
    @ViewChild('tableCenterH_border')
    tableCenterHBorder: Button;
    @ViewChild('tableBottom_border')
    tableBottomBorder: Button;
    @ViewChild('borderSize')
    tableBorderSize: DropDownButton;
    @ViewChild('topMargin')
    tableTopMargin: NumericTextBox;
    @ViewChild('bottomMargin')
    tableBottomMargin: NumericTextBox;
    @ViewChild('leftMargin')
    tableLeftMargin: NumericTextBox;
    @ViewChild('rightMargin')
    tableRightMargin: NumericTextBox;
    @ViewChild('table_tab')
    tabComponent: TabComponent;
    //Initializes property pane.
    // public tocProperties: TocPropertiesModel;
    // public headerFooter: HeaderFooterProperties;
    // public fontProperties: TextPropertiesModel;
    // public imageProperties: ImageProperties;
    // public tableProperties: TableProperties;
    // public propertiesPane: PropertiesPane;
    //Initializes document editor toolbar and events.    
    public titleBar: TitleBar;
    public statusBar: StatusBar;
    //Toolbar Implementation
    @ViewChild('de_toolbar')
    public toolBarModule: ToolbarComponent;
    public containerPanel: HTMLElement;
    public isReadOnly: boolean = false;
    public showPropertiesPane: boolean = true;
    public previousContext: ContextType;
    public showHeaderProperties: boolean = false;
    public file: HTMLInputElement;
    public insertImage: HTMLInputElement;
    public documentLoader: DocumentLoader;
    public isContentChange: boolean = false;
    private fileNameTextBox: HTMLInputElement;
    public newDialog: Dialog;
    public templateLoader: TemplateLoader;
    public showHideButton: HTMLButtonElement;
    public togglePropPane: Button
    private isComponentCreated: boolean = false;
    private isRetrieving: boolean = false;

    private isTemplateEditing: boolean = false;

    public textProperties: TextPropertiesModel = new TextPropertiesModel(this);
    public paragraphProperties: ParagraphPropertiesModel = new ParagraphPropertiesModel(this);
    public imageProperties: ImagePropertiesModel = new ImagePropertiesModel();
    public tableProperties: TablePropertiesModel = new TablePropertiesModel(this);
    public headerFooterProperties: HeaderFooterPropertiesModel = new HeaderFooterPropertiesModel(this);
    public tocProperties: TocPropertiesModel = new TocPropertiesModel(this);
    public tableTextProperties: TableTextPropertiesModel = new TableTextPropertiesModel(this);
    public tableParagraphProperties: TableParagraphPropertiesModel = new TableParagraphPropertiesModel(this);
    public propertiesPane: PropertiesPaneModel = new PropertiesPaneModel();
    public insertFieldProperties: InsertFieldPropertiesModel = new InsertFieldPropertiesModel(this);

    @ViewChild('heightNumericBox')
    public heightNumericBox: NumericTextBox;
    @ViewChild('widthNumericBox')
    public widthNumericBox: NumericTextBox;
    @ViewChild('headerFromTop')
    public headerFromTopNumericBox: NumericTextBox;
    @ViewChild('footerFromTop')
    public footerFromTopNumericBox: NumericTextBox;
    appliedLastBullet: string = 'dot';
    appliedLastNumbering: string = 'arabic';
    appliedHighlightColor: string = 'rgb(255, 255, 0)';

    private cntnt = '';

    constructor(private httpClient: HttpClient, public documentEditorService: DocumentEditorService) {}
    
    public nodePropertyChange(args: { [key: string]: Object }): void {
        if (!this.isRetrieving) {
            switch (args.propertyName.toString().toLowerCase()) {
                case 'fontname':
                    this.documentEditor.selection.characterFormat.fontFamily = args.propertyValue as string;
                    break;
                case 'fontsize':
                    this.documentEditor.selection.characterFormat.fontSize = args.propertyValue as number;
                    break;
                case 'bold':
                    this.documentEditor.editorModule.toggleBold();
                    break;
                case 'italic':
                    this.documentEditor.editorModule.toggleItalic();
                    break;
                case 'underline':
                    this.documentEditor.editorModule.toggleUnderline('Single');
                    break;
                case 'strikethrough':
                    this.documentEditor.editorModule.toggleStrikethrough();
                    break;
                case 'superscript':
                    this.documentEditor.editorModule.toggleSuperscript();
                    break;
                case 'subscript':
                    this.documentEditor.editorModule.toggleSubscript();
                    break;
                case 'fontcolor':
                    this.documentEditor.selection.characterFormat.fontColor = args.propertyValue as string;
                    break;
                case 'highlightcolor':
                    this.documentEditor.selection.characterFormat.highlightColor = this.getHighLightColor(args.propertyValue as string);
                    break;
                case 'propertypaneshow':
                    setTimeout(() => {
                        this.documentEditor.resize();
                    });
                    break;
            }
        }
        setTimeout(() => {
            this.documentEditor.focusIn();
        });
    }
    getHighLightColor(color: string): HighlightColor {
        switch (color) {
            case 'rgb(255, 255, 0)':
                return 'Yellow';
            case 'rgb(0, 255, 0)':
                return 'BrightGreen';
            case 'rgb(0, 255, 255)':
                return 'Turquoise';
            case 'rgb(255, 0, 255)':
                return 'Pink';
            case 'rgb(0, 0, 255)':
                return 'Blue';
            case 'rgb(255, 0, 0)':
                return 'Red';
            case 'rgb(0, 0, 128)':
                return 'DarkBlue';
            case 'rgb(0, 128, 128)':
                return 'Teal';
            case 'rgb(0, 128, 0)':
                return 'Green';
            case 'rgb(128, 0, 128)':
                return 'Violet';
            case 'rgb(128, 0, 0)':
                return 'DarkRed';
            case 'rgb(128, 128, 0)':
                return 'DarkYellow';
            case 'rgb(128, 128, 128)':
                return 'Gray50';
            case 'rgb(192, 192, 192)':
                return 'Gray25';
            case 'rgb(0, 0, 0)':
                return 'Black';
            default:
                return 'NoColor';
        }
    }
    public ngOnInit(): void {
        this.documentEditor.acceptTab = true;
        this.documentEditor.pageOutline = '#E0E0E0';
        this.containerPanel = document.getElementById('documenteditor_container_body');
        this.textProperties.propertyChange = this.nodePropertyChange.bind(this);
        this.tableTextProperties.propertyChange = this.nodePropertyChange.bind(this);
        this.paragraphProperties.propertyChange = this.nodePropertyChange.bind(this);
        this.propertiesPane.propertyChange = this.nodePropertyChange.bind(this);
        this.imageProperties.documentEditor = this.documentEditor;
        this.imageProperties.heightNumericBox = this.heightNumericBox;
        this.imageProperties.widthNumericBox = this.widthNumericBox;
        document.getElementById('container_widthInput').addEventListener('keydown', this.imageProperties.onImageWidth);
        document.getElementById('container_heightInput').addEventListener('keydown', this.imageProperties.onImageHeight);
        document.getElementById('container_widthInput').addEventListener('blur', (): void => { this.imageProperties.applyImageWidth(); this.imageProperties.isWidthApply = false; });
        document.getElementById('container_heightInput').addEventListener('blur', (): void => { this.imageProperties.applyImageHeight(); this.imageProperties.isHeightApply = false; });

        this.tocProperties.documentEditor = this.documentEditor;
        this.tocProperties.initializeTocPane();

        this.insertFieldProperties.loadFields();
    }


    public initHelperModules(): void {
        this.documentLoader = new DocumentLoader(this.documentEditor);
        this.templateLoader = new TemplateLoader();
        if (!this.isReadOnly) {
            this.updateUndoRedoBtn();
        }
        document.getElementById('documenteditor_titlebar').style.display = '';
        document.getElementById('documenteditor_statusbar').style.display = '';
        this.titleBar = new TitleBar(document.getElementById('documenteditor_titlebar'), this.documentEditor, true, this.httpClient);
        this.statusBar = new StatusBar(document.getElementById('documenteditor_statusbar'), this.documentEditor);
        this.updateContainerSize();
        this.documentEditor.resize();
        window.addEventListener('resize', (): void => { this.onWindowResize(); });
        if (!this.isReadOnly) {
            this.updateUndoRedoBtn();
        }
        this.onLoadDefault();
        this.applyPageCountAndDocumentTitle();
        this.previousContext = this.documentEditor.selection.contextType;
        this.onUseLocalClipboard();
        let leftDiv2 = document.getElementById("font_properties_color");
        (leftDiv2.firstElementChild.lastElementChild.lastElementChild.firstChild as HTMLElement).classList.add('e-de-icon-FontColor', 'e-font-icons');
        let tableLeftDiv2 = document.getElementById("table_font_properties_color");
        (tableLeftDiv2.firstElementChild.lastElementChild.lastElementChild.firstChild as HTMLElement).classList.add('e-de-icon-FontColor', 'e-font-icons');
        this.highlightColor.element.nextElementSibling.firstElementChild.classList.add('e-de-icon-HighlightColor');
        this.tableHighlightColor.element.nextElementSibling.firstElementChild.classList.add('e-de-icon-HighlightColor');
        this.paragraphProperties.updateStyleNames();
        this.tableParagraphProperties.updateStyleNames();
        document.getElementById('container_topMargin').addEventListener('keydown', this.tableProperties.onTopMargin);
        document.getElementById('container_topMargin').addEventListener('blur', (): void => { this.tableProperties.applyTopMargin(); this.tableProperties.isTopMarginApply = false; });

        document.getElementById('container_bottomMargin').addEventListener('keydown', this.tableProperties.onBottomMargin);
        document.getElementById('container_bottomMargin').addEventListener('blur', (): void => { this.tableProperties.applyBottomMargin(); this.tableProperties.isBottomMarginApply = false; });

        document.getElementById('container_leftMargin').addEventListener('keydown', this.tableProperties.onLeftMargin);
        document.getElementById('container_leftMargin').addEventListener('blur', (): void => { this.tableProperties.applyLeftMargin(); this.tableProperties.isLeftMarginApply = false; });

        document.getElementById('container_rightMargin').addEventListener('keydown', this.tableProperties.onRightMargin);
        document.getElementById('container_rightMargin').addEventListener('blur', (): void => { this.tableProperties.applyRightMargin(); this.tableProperties.isRightMarginApply = false; });
    }
    public documentChange = (): void => {
        if (!this.isComponentCreated) {
            this.isComponentCreated = true;
            this.updateContainerSize();
            this.documentEditor.resize();
            setTimeout(() => {
                this.initHelperModules();
            });
        }
        this.updateUndoRedoBtn();
        this.isContentChange = false;
        this.applyPageCountAndDocumentTitle();
    };
    public selectionChange = () => {
        setTimeout(() => {
            this.onSelectionChange();
        }, 20);
    };
    public contentChange = (): void => {
        this.isContentChange = true;
        if (!this.isReadOnly) {
            this.updateUndoRedoBtn();
        }
        //Set page count
        if(this.statusBar) {
            this.statusBar.updatePageCount();
        }
    };
    public requestNavigate = (args: RequestNavigateEventArgs) => {
        if (args.linkType !== 'Bookmark') {
            let link: string = args.navigationLink;
            if (args.localReference.length > 0) {
                link += '#' + args.localReference;
            }
            window.open(link);
            args.isHandled = true;
        }
    };
    public zoomFactorChange = (): void => {
        this.statusBar.updateZoomContent();
    };
    public viewChange = (e: ViewChangeEventArgs): void => {
        this.statusBar.updatePageNumberOnViewChange(e);
    };
    private onValueChange(args: any): void {
        this.documentEditor.zoomFactor = parseInt(args.value as any, 10) / 100;
    }
    private applyPageCountAndDocumentTitle(): void {
        //Sets Document name.
        if (this.titleBar && this.statusBar) {
            this.titleBar.updateDocumentTitle();
            this.statusBar.updatePageCount();
        }
    }
    public updateContainerSize(): void {
        let titleBarDiv: HTMLElement = document.getElementById('documenteditor_titlebar');
        let statusBarDiv: HTMLElement = document.getElementById('documenteditor_statusbar');
        let toolBarDiv: HTMLElement = document.getElementById('documenteditor_toolbar');
        if (this.containerPanel && titleBarDiv && statusBarDiv && toolBarDiv) {
            this.containerPanel.style.height = window.innerHeight - (titleBarDiv.offsetHeight
                + toolBarDiv.offsetHeight + statusBarDiv.offsetHeight) + 'px';
        }
    }
    private onSelectionChange(): void {
        if (this.documentEditor.selection) {
            this.showPropertiesPaneOnSelection();

            if (this.statusBar) {
                this.statusBar.startPage = this.documentEditor.selection.startPage;
                this.statusBar.updatePageNumber();
            }
        }
        this.isRetrieving = false;
    }
    private onLoadDefault(): void {
        // tslint:disable
        // let defaultDocument: object = { "sections": [{ "sectionFormat": { "pageWidth": 612, "pageHeight": 792, "leftMargin": 72, "rightMargin": 72, "topMargin": 72, "bottomMargin": 72, "differentFirstPage": false, "differentOddAndEvenPages": false, "headerDistance": 36, "footerDistance": 36 }, "blocks": [{ "paragraphFormat": { "afterSpacing": 30, "styleName": "Heading 1", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Adventure Works Cycles" }] }, { "paragraphFormat": { "firstLineIndent": 36, "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Adventure Works Cycles, the fictitious company on which the " }, { "characterFormat": {}, "text": "AdventureWorks" }, { "characterFormat": {}, "text": " sample databases " }, { "characterFormat": {}, "text": "are based, is a large, multinational manufacturing company. The company manufactures and sells metal " }, { "characterFormat": {}, "text": "and composite bicycles to North American, European and Asian commercial markets. While its base " }, { "characterFormat": {}, "text": "operation " }, { "characterFormat": {}, "text": "is located in" }, { "characterFormat": {}, "text": " Bothell, Washington with 290 employees, several regional sales teams are located " }, { "characterFormat": {}, "text": "throughout their market base." }] }, { "paragraphFormat": { "firstLineIndent": 36, "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "In 2000, Adventure Works Cycles bought a small manufacturing plant, " }, { "characterFormat": {}, "text": "Importadores" }, { "characterFormat": {}, "text": " Neptuno, " }, { "characterFormat": {}, "text": "located in Mexico. " }, { "characterFormat": {}, "text": "Importadores" }, { "characterFormat": {}, "text": " Neptuno manufactures several critical subcomponents for the " }, { "characterFormat": {}, "text": "Adventure Works Cycles product line. These subcomponents are shipped to the Bothell location for final " }, { "characterFormat": {}, "text": "product assembly. In 2001, " }, { "characterFormat": {}, "text": "Importadores" }, { "characterFormat": {}, "text": " Neptuno, became the sole manufacturer and distributor of the " }, { "characterFormat": {}, "text": "touring bicycle product group." }] }, { "paragraphFormat": { "styleName": "Heading 1", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Product Overview" }] }, { "rows": [{ "cells": [{ "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": { "fontSize": "11" }, "inlines": [{ "characterFormat": { "fontSize": "11" }, "imageString": "data:image/gif;base64,", "width": 141.99998, "height": 88.5 }] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 0 }, { "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Heading 2", "outlineLevel": "Level2", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Mountai" }, { "characterFormat": {}, "bookmarkType": 0, "name": "_GoBack" }, { "characterFormat": {}, "bookmarkType": 1, "name": "_GoBack" }, { "characterFormat": {}, "text": "n-200" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Product No: BK-M68B-38" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Size: 38" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Weight: 25" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Price: $2,294.99" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 1 }], "rowFormat": { "height": 1, "allowBreakAcrossPages": true, "heightType": "AtLeast", "isHeader": false, "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "gridBefore": 0, "gridBeforeWidth": 0, "gridBeforeWidthType": "Point", "gridAfter": 0, "gridAfterWidth": 0, "gridAfterWidthType": "Point" } }, { "cells": [{ "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Heading 2", "outlineLevel": "Level2", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Mountain-300" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Product No: BK-M47B-38" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Size: 35" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Weight: 22" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Price: $1,079.99" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 0 }, { "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "imageString": "data:image/gif;base64,", "width": 135, "height": 84 }] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 1 }], "rowFormat": { "height": 1, "allowBreakAcrossPages": true, "heightType": "AtLeast", "isHeader": false, "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "gridBefore": 0, "gridBeforeWidth": 0, "gridBeforeWidthType": "Point", "gridAfter": 0, "gridAfterWidth": 0, "gridAfterWidthType": "Point" } }, { "cells": [{ "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "imageString": "data:image/gif;base64,", "width": 165.5, "height": 103 }] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 0 }, { "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Heading 2", "outlineLevel": "Level2", "listFormat": {} }, "characterFormat": { "fontSize": "13" }, "inlines": [{ "characterFormat": {}, "text": "Road-150" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Product No: BK-R93R-44" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Size: 44" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Weight: 14" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Price: $3,578.27" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 1 }], "rowFormat": { "height": 1, "allowBreakAcrossPages": true, "heightType": "AtLeast", "isHeader": false, "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "gridBefore": 0, "gridBeforeWidth": 0, "gridBeforeWidthType": "Point", "gridAfter": 0, "gridAfterWidth": 0, "gridAfterWidthType": "Point" } }], "grid": [234, 234], "tableFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "cellSpacing": 0, "leftIndent": 0, "tableAlignment": "Left", "topMargin": 0, "rightMargin": 5.4, "leftMargin": 5.4, "bottomMargin": 0, "preferredWidth": 0, "preferredWidthType": "Auto" }, "description": null, "title": null }, { "paragraphFormat": { "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [] }], "headersFooters": { "header": { "blocks": [{ "paragraphFormat": { "listFormat": {} }, "characterFormat": {}, "inlines": [] }] }, "footer": { "blocks": [{ "paragraphFormat": { "listFormat": {} }, "characterFormat": {}, "inlines": [] }] } } }], "characterFormat": { "fontSize": 11, "fontFamily": "Calibri" }, "paragraphFormat": { "lineSpacing": 1.0791666507720947, "lineSpacingType": "Multiple", "listFormat": {} }, "styles": [{ "name": "Normal", "type": "Paragraph", "paragraphFormat": { "lineSpacing": 1.149999976158142, "lineSpacingType": "Multiple", "listFormat": {} }, "characterFormat": { "fontFamily": "Calibri" }, "next": "Normal" }, { "name": "Default Paragraph Font", "type": "Character", "characterFormat": {} }, { "name": "Heading 1 Char", "type": "Character", "characterFormat": { "fontSize": 16, "fontFamily": "Calibri Light", "fontColor": "#2F5496" }, "basedOn": "Default Paragraph Font" }, { "name": "Heading 1", "type": "Paragraph", "paragraphFormat": { "beforeSpacing": 12, "afterSpacing": 0, "outlineLevel": "Level1", "listFormat": {} }, "characterFormat": { "fontSize": 16, "fontFamily": "Calibri Light", "fontColor": "#2F5496" }, "basedOn": "Normal", "link": "Heading 1 Char", "next": "Normal" }, { "name": "Heading 2 Char", "type": "Character", "characterFormat": { "fontSize": 13, "fontFamily": "Calibri Light", "fontColor": "#2F5496" }, "basedOn": "Default Paragraph Font" }, { "name": "Heading 2", "type": "Paragraph", "paragraphFormat": { "beforeSpacing": 2, "afterSpacing": 6, "outlineLevel": "Level2", "listFormat": {} }, "characterFormat": { "fontSize": 13, "fontFamily": "Calibri Light", "fontColor": "#2F5496" }, "basedOn": "Normal", "link": "Heading 2 Char", "next": "Normal" }], "lists": [], "abstractLists": [] };
        let defaultDocument: object = {"sections":[{"sectionFormat":{"pageWidth":612,"pageHeight":792,"leftMargin":72,"rightMargin":72,"topMargin":72,"bottomMargin":72,"differentFirstPage":false,"differentOddAndEvenPages":false,"headerDistance":36,"footerDistance":36},"blocks":[{"paragraphFormat":{"styleName":"Normal","listFormat":{}},"characterFormat":{},"inlines":[]}],"headersFooters":{"header":{"blocks":[{"paragraphFormat":{"listFormat":{}},"characterFormat":{},"inlines":[]}]},"footer":{"blocks":[{"paragraphFormat":{"listFormat":{}},"characterFormat":{},"inlines":[]}]},"evenHeader":{},"evenFooter":{},"firstPageHeader":{},"firstPageFooter":{}}}],"characterFormat":{},"paragraphFormat":{"listFormat":{}},"styles":[{"name":"Normal","type":"Paragraph","paragraphFormat":{"listFormat":{}},"characterFormat":{},"next":"Normal"},{"name":"Heading 1","type":"Paragraph","paragraphFormat":{"leftIndent":0,"rightIndent":0,"firstLineIndent":0,"textAlignment":"Left","beforeSpacing":12,"afterSpacing":0,"lineSpacing":1.0791666507720947,"lineSpacingType":"Multiple","outlineLevel":"Level1","listFormat":{}},"characterFormat":{"fontSize":16,"fontFamily":"Calibri Light","fontColor":"#2F5496"},"basedOn":"Normal","link":"Heading 1 Char","next":"Normal"},{"name":"Heading 1 Char","type":"Character","characterFormat":{"fontSize":16,"fontFamily":"Calibri Light","fontColor":"#2F5496"},"basedOn":"Default Paragraph Font"},{"name":"Default Paragraph Font","type":"Character","characterFormat":{}},{"name":"Heading 2","type":"Paragraph","paragraphFormat":{"leftIndent":0,"rightIndent":0,"firstLineIndent":0,"textAlignment":"Left","beforeSpacing":2,"afterSpacing":0,"lineSpacing":1.0791666507720947,"lineSpacingType":"Multiple","outlineLevel":"Level2","listFormat":{}},"characterFormat":{"fontSize":13,"fontFamily":"Calibri Light","fontColor":"#2F5496"},"basedOn":"Normal","link":"Heading 2 Char","next":"Normal"},{"name":"Heading 2 Char","type":"Character","characterFormat":{"fontSize":13,"fontFamily":"Calibri Light","fontColor":"#2F5496"},"basedOn":"Default Paragraph Font"},{"name":"Heading 3","type":"Paragraph","paragraphFormat":{"leftIndent":0,"rightIndent":0,"firstLineIndent":0,"textAlignment":"Left","beforeSpacing":2,"afterSpacing":0,"lineSpacing":1.0791666507720947,"lineSpacingType":"Multiple","outlineLevel":"Level3","listFormat":{}},"characterFormat":{"fontSize":12,"fontFamily":"Calibri Light","fontColor":"#1F3763"},"basedOn":"Normal","link":"Heading 3 Char","next":"Normal"},{"name":"Heading 3 Char","type":"Character","characterFormat":{"fontSize":12,"fontFamily":"Calibri Light","fontColor":"#1F3763"},"basedOn":"Default Paragraph Font"},{"name":"Heading 4","type":"Paragraph","paragraphFormat":{"leftIndent":0,"rightIndent":0,"firstLineIndent":0,"textAlignment":"Left","beforeSpacing":2,"afterSpacing":0,"lineSpacing":1.0791666507720947,"lineSpacingType":"Multiple","outlineLevel":"Level4","listFormat":{}},"characterFormat":{"italic":true,"fontFamily":"Calibri Light","fontColor":"#2F5496"},"basedOn":"Normal","link":"Heading 4 Char","next":"Normal"},{"name":"Heading 4 Char","type":"Character","characterFormat":{"italic":true,"fontFamily":"Calibri Light","fontColor":"#2F5496"},"basedOn":"Default Paragraph Font"},{"name":"Heading 5","type":"Paragraph","paragraphFormat":{"leftIndent":0,"rightIndent":0,"firstLineIndent":0,"textAlignment":"Left","beforeSpacing":2,"afterSpacing":0,"lineSpacing":1.0791666507720947,"lineSpacingType":"Multiple","outlineLevel":"Level5","listFormat":{}},"characterFormat":{"fontFamily":"Calibri Light","fontColor":"#2F5496"},"basedOn":"Normal","link":"Heading 5 Char","next":"Normal"},{"name":"Heading 5 Char","type":"Character","characterFormat":{"fontFamily":"Calibri Light","fontColor":"#2F5496"},"basedOn":"Default Paragraph Font"},{"name":"Heading 6","type":"Paragraph","paragraphFormat":{"leftIndent":0,"rightIndent":0,"firstLineIndent":0,"textAlignment":"Left","beforeSpacing":2,"afterSpacing":0,"lineSpacing":1.0791666507720947,"lineSpacingType":"Multiple","outlineLevel":"Level6","listFormat":{}},"characterFormat":{"fontFamily":"Calibri Light","fontColor":"#1F3763"},"basedOn":"Normal","link":"Heading 6 Char","next":"Normal"},{"name":"Heading 6 Char","type":"Character","characterFormat":{"fontFamily":"Calibri Light","fontColor":"#1F3763"},"basedOn":"Default Paragraph Font"}],"lists":[],"abstractLists":[]};
        // tslint: enable
        let waitingPopUp: HTMLElement = document.getElementById('waiting-popup');
        let popupOverlay: HTMLElement = document.getElementById('popup-overlay');
        waitingPopUp.style.display = 'block';
        this.documentEditor.open(JSON.stringify(defaultDocument));
        this.documentEditor.documentName = 'New';
        waitingPopUp.style.display = 'none';
        popupOverlay.style.display = 'none';
        this.documentEditor.focusIn();
    }
    private onWindowResize(): void {
        this.updateContainerSize();
    }
    //open new tab
    public newTabClick(): void {
        // tslint:disable-next-line:max-line-length
        document.getElementById('newTab').setAttribute('href', location.href.split('#')[0] + 'samples/documenteditor/default/index.html#fabric');
    }
    //#region Toolbar 
    public onToolbarClick = (args: ClickEventArgs) => {
        switch (args.item.id) {
            case 'new':
                this.insertFieldProperties.show = false;
                this.isTemplateEditing = false;
                this.titleBar.saveTemplate=false;
                this.enableIcons();
                this.newDocument();
                break;
            case 'open':
                //opens file explorer
                this.insertFieldProperties.show = false;             
                this.file.value = '';
                this.file.click();
                break;
            case 'undo':
                this.documentEditor.editorHistory.undo();
                break;
            case 'redo':
                this.documentEditor.editorHistory.redo();
                break;
            case 'image':
                this.insertImage.value = '';
                this.insertImage.click();
                break;
            case 'table':
                this.documentEditor.showDialog('Table');
                break;
            case 'link':
                this.documentEditor.showDialog('Hyperlink');
                break;
            case 'bookmark':
                this.documentEditor.showDialog('Bookmark');
                break;
            case 'pagesetup':
                this.documentEditor.showDialog('PageSetup');
                break;
            case 'pagenumber':
                this.documentEditor.editor.insertPageNumber();
                break;
            case 'find':
                this.documentEditor.showOptionsPane();
                break;
            case 'hideProperty':
                this.onPropertyPaneClick();
                break;
            case 'header':
                this.showHeaderProperties = true;
                this.documentEditor.selection.goToHeader();
                break;
            case 'footer':
                this.showHeaderProperties = true;
                this.documentEditor.selection.goToFooter();
                break;
            case 'toc':
                this.onToc();
                break;
            case 'restrictEdit':
                this.onRestrictButtonClick();
                break;
            case 'useLocalClipboard':
                this.onUseLocalClipboard();
                break;
            case 'newtemplate':
                this.insertFieldProperties.show = false;
                this.isTemplateEditing=true;
                this.newDocument();
                break;
            case 'field':
                this.insertField();
                break;
        }
    }
    public toolBarRendered = (): void => {
        this.initializeFileTypeElements();
        this.initializeCreateNewDocumentDialog();
        // tslint:disable-next-line:max-line-length
        let divElement: HTMLElement = createElement('div', { styles: 'float: right;width: auto;background-color: #fff;position:absolute;top:63px;right:20px;' });
        this.showHideButton = createElement('button', {
            className: 'e-de-togglebutton',
            id: 'showHidePane',
            styles: 'float:right;box-shadow: none;position:relative;background:transparent;border:none;'
        }) as HTMLButtonElement;
        this.showHideButton.setAttribute('title', 'Show or hide property pane.');
        divElement.appendChild(this.showHideButton);
        this.toolBarModule.element.parentElement.appendChild(divElement);
        this.togglePropPane = new Button({ iconCss: 'e-de-icon-ShowHidePane' }, this.showHideButton);
        this.togglePropPane.element.addEventListener('click', this.onPropertyPaneClick);
        (document.getElementById('showHidePane').firstChild as HTMLElement).classList.add('e-pane-enabled');

        let tocElement: HTMLDivElement = document.getElementById('toc').getElementsByTagName('div')[0];
        this.onTextWrapTOCButton(tocElement);
        let pageSetupElement: HTMLDivElement = document.getElementById('pagesetup').getElementsByTagName('div')[0];
        this.onTextWrap(pageSetupElement);
        let pageNumberElement: HTMLDivElement = document.getElementById('pagenumber').getElementsByTagName('div')[0];
        this.onTextWrap(pageNumberElement);
        let localClipboardElement: HTMLDivElement = document.getElementById('useLocalClipboard').getElementsByTagName('div')[0];
        this.onTextWrap(localClipboardElement);
        let restrictEditElement: HTMLDivElement = document.getElementById('restrictEdit').getElementsByTagName('div')[0];
        this.onTextWrap(restrictEditElement);
    }
    private newDocument = (): void => {
        //Adds new document.
        //this.documentEditor.newDocument();
        if (this.isContentChange) {
            let msg: boolean = confirm('Do you want to save the changes to ' + this.documentEditor.documentName + '?');
            if (msg === true) {
                this.documentEditor.save(this.documentEditor.documentName, 'Sfdt');
            }
        }
        this.showCreateNewDocumentdialog();
        this.templateSelection(null);
        let templates: any = document.getElementsByClassName('template');
        for (let i: number = 0; i < templates.length; i++) {
            templates[i].addEventListener('click', (e: Event) => { this.templateSelection(e); });
        }
    }
    
    /* tslint:disable:no-any */
    private onFileChange = (args: any): void => {
        if (args.target.files[0]) {
            let path: any = args.target.files[0];
            if (path.name.substr(path.name.lastIndexOf('.')) === '.sfdt') {
                let fileReader: FileReader = new FileReader();
                fileReader.onload = (e: any) => {
                    let contents: any = e.target.result;
                    this.documentEditor.open(contents);
                };
                fileReader.readAsText(path);
                this.documentEditor.documentName = path.name.substr(0, path.name.lastIndexOf('.'));
            } else {
                this.documentLoader.loadFile(path);
            }
        }
        event.preventDefault();
    }
    /**
  * Inserts image.
  */
    private onInsertImage = (args: any): void => {
        let documenteditor: DocumentEditorComponent = this.documentEditor;
        if (navigator.userAgent.match('Chrome') || navigator.userAgent.match('Firefox') || navigator.userAgent.match('Edge')
            || navigator.userAgent.match('MSIE') || navigator.userAgent.match('.NET')) {
            if (args.target.files[0]) {
                let path: any = args.target.files[0];
                let reader: FileReader = new FileReader();
                reader.onload = (frEvent: any): void => {
                    let base64String: any = frEvent.target.result;
                    let image: HTMLImageElement = document.createElement('img');
                    image.addEventListener('load', (): void => {
                        documenteditor.editor.insertImage(base64String, image.width, image.height);
                    });
                    image.src = base64String;
                };
                reader.readAsDataURL(path);
            }
            //Safari does not Support FileReader Class
        } else {
            let image: HTMLImageElement = document.createElement('img');
            image.addEventListener('load', (): void => {
                documenteditor.editor.insertImage(args.target.value);
            });
            image.src = args.target.value;
        }
    }
    private onTextWrapTOCButton(btn: HTMLDivElement): void {
        let text: string = btn.innerHTML;
        let index: number = text.indexOf('C');
        btn.innerHTML = text.slice(0, index);
        let divElement: HTMLDivElement = createElement('div', { className: 'e-de-text-wrap' }) as HTMLDivElement;
        divElement.innerHTML = text.slice(index, text.length);
        btn.appendChild(divElement);
    }
    private onTextWrap(btn: HTMLDivElement): void {
        let text: string = btn.innerHTML;
        let index: number = text.indexOf(' ');
        btn.innerHTML = text.slice(0, index);
        let splitChar: string[] = text.split(' ');
        for (let i: number = 1; i < splitChar.length; i++) {
            let divElement: HTMLDivElement = createElement('div', { className: 'e-de-text-wrap' }) as HTMLDivElement;
            divElement.innerHTML = splitChar[i];
            btn.appendChild(divElement);
        }
    }
    private onRestrictButtonClick = (): void => {
        this.isReadOnly = !this.isReadOnly;
        if (this.isReadOnly) {
            this.disableIcons();
        } else {
            this.enableIcons();
        }
        this.documentEditor.focusIn();
    }
    private onToc = (): void => {
        if (this.previousContext === 'TableOfContents' && this.showPropertiesPane) {
            this.documentEditor.focusIn();
            return;
        }
        if (this.headerFooterProperties.show) {
            this.documentEditor.selection.closeHeaderFooter();
        }
        this.enableDisablePropertyPaneButton(false);
        this.propertiesPane.show = true;
        this.showProperties('toc');
    }
    private enableIcons = (): void => {
        this.documentEditor.isReadOnly = false;
        this.updateUndoRedoBtn();
        this.toolBarModule.enableItems(document.getElementById('image').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('table').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('link').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('bookmark').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('toc').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('header').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('footer').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('pagesetup').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('pagenumber').parentElement, true);
        this.toolBarModule.enableItems(document.getElementById('useLocalClipboard').parentElement, true);
        document.getElementById('showHidePane').parentElement.className = '';
        document.getElementById('restrictEdit').className = 'e-tbar-btn e-tbtn-txt e-control e-btn';
        document.getElementById('useLocalClipboard').className = 'e-tbar-btn e-tbtn-txt e-control e-btn e-btn-toggle';
        this.showPropertiesPane = true;
        this.showPropertiesPaneOnSelection();
    }
    private disableIcons = (): void => {
        this.documentEditor.isReadOnly = true;
        this.toolBarModule.enableItems(document.getElementById('undo').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('redo').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('image').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('table').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('link').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('bookmark').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('toc').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('header').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('footer').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('pagesetup').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('pagenumber').parentElement, false);
        this.toolBarModule.enableItems(document.getElementById('useLocalClipboard').parentElement, false);
        this.propertiesPane.show = false;
        this.showPropertiesPane = false;
        document.getElementById('showHidePane').parentElement.className = 'e-overlay';
        document.getElementById('restrictEdit').className = 'e-tbar-btn e-tbtn-txt e-control e-btn e-btn-toggle';
    }
    private onPropertyPaneClick = (): void => {
        if (this.previousContext === 'TableOfContents' && this.showPropertiesPane) {
            this.documentEditor.focusIn();
            return;
        } else if (this.tocProperties.show && this.showPropertiesPane) {
            this.enableDisablePropertyPaneButton(true);
            this.showPropertiesPaneOnSelection();
            return;
        }
        if (this.previousContext.indexOf('Header') >= 0
            || this.previousContext.indexOf('Footer') >= 0) {
            this.showHeaderProperties = !this.showHeaderProperties;
        } else {
            this.showPropertiesPane = !this.showPropertiesPane;
        }
        this.showPropertiesPaneOnSelection();
        setTimeout(() => {
            this.updateContainerSize();
            this.documentEditor.resize();
        }, 20);
        this.showPropertiesPane = this.propertiesPane.show;
        this.enableDisablePropertyPaneButton(this.showPropertiesPane);
        this.documentEditor.focusIn();
    }
    public enableDisablePropertyPaneButton(isShow: boolean): void {
        if (isShow) {
            (this.showHideButton.firstChild as HTMLElement).classList.remove('e-pane-disabled');
            (this.showHideButton.firstChild as HTMLElement).classList.add('e-pane-enabled');
        } else {
            (this.showHideButton.firstChild as HTMLElement).classList.add('e-pane-disabled');
            (this.showHideButton.firstChild as HTMLElement).classList.remove('e-pane-enabled');
        }
    }
    public showPropertiesPaneOnSelection = (): void => {
        let currentContext: string = this.documentEditor.selection.contextType;

        if (this.insertFieldProperties.show === true)
        {
              return;
        }

        let isInHeaderFooter: boolean = currentContext.indexOf('Header') >= 0
            || currentContext.indexOf('Footer') >= 0;
        if (!isInHeaderFooter && !this.showPropertiesPane) {
            this.propertiesPane.show = false;
            this.documentEditor.focusIn();
            return;
        }
        if (!isInHeaderFooter) {
            if (this.headerFooterProperties.show) {
                this.propertiesPane.show = false;
                this.documentEditor.selection.closeHeaderFooter();
            }
            this.showHeaderProperties = true;
        } else if (isInHeaderFooter && this.showHeaderProperties) {
            this.showPropertiesPane = true;
        }
        if (this.showPropertiesPane) {
            this.propertiesPane.show = true;
            if (isInHeaderFooter && this.showHeaderProperties) {
                this.showProperties('headerfooter');
            } else {
                if (currentContext.indexOf('Text') >= 0
                    && currentContext.indexOf('Table') < 0) {
                    this.showProperties('text');
                } else if (currentContext.indexOf('Image') >= 0) {
                    this.showProperties('image');
                } else if (currentContext.indexOf('TableOfContents') >= 0) {
                    this.showProperties('toc');
                } else if (currentContext.indexOf('Table') >= 0) {
                    this.showProperties('table');
                }
            }
        }
        this.previousContext = this.documentEditor.selection.contextType;
    }
    private updateTextModel() {
        this.textProperties.isBold = this.documentEditor.selection.characterFormat.bold;
        this.textProperties.isItalic = this.documentEditor.selection.characterFormat.italic;
        this.textProperties.isUnderline = this.documentEditor.selection.characterFormat.underline !== 'None';
        this.textProperties.isStrikethrough = this.documentEditor.selection.characterFormat.strikethrough !== 'None';
        this.textProperties.isSuperscript = this.documentEditor.selection.characterFormat.baselineAlignment === 'Superscript';
        this.textProperties.isSubscript = this.documentEditor.selection.characterFormat.baselineAlignment === 'Subscript';
        this.textProperties.fontName = this.documentEditor.selection.characterFormat.fontFamily;
        this.textProperties.fontSize = this.documentEditor.selection.characterFormat.fontSize;
        this.textProperties.fontColor = this.documentEditor.selection.characterFormat.fontColor;
        (this.highlightColor.element.firstElementChild as HTMLElement).style.backgroundColor = this.appliedHighlightColor;
        this.textProperties.applyHighlightColorAsBackground(this.documentEditor.selection.characterFormat.highlightColor);
        this.paragraphProperties.alignLeft = this.documentEditor.selection.paragraphFormat.textAlignment === 'Left';
        this.paragraphProperties.alignRight = this.documentEditor.selection.paragraphFormat.textAlignment === 'Right';
        this.paragraphProperties.alignCenter = this.documentEditor.selection.paragraphFormat.textAlignment === 'Center';
        this.paragraphProperties.alignJustify = this.documentEditor.selection.paragraphFormat.textAlignment === 'Justify';
        let styleName: string = this.documentEditor.selection.paragraphFormat.styleName;
        if (styleName) {
            this.styleDropDown.value = styleName;
        } else {
            this.styleDropDown.value = '';
        }

    }
    public updateTableTextModel(): void {
        this.tableTextProperties.isBold = this.documentEditor.selection.characterFormat.bold;
        this.tableTextProperties.isItalic = this.documentEditor.selection.characterFormat.italic;
        this.tableTextProperties.isUnderline = this.documentEditor.selection.characterFormat.underline !== 'None';
        this.tableTextProperties.isStrikethrough = this.documentEditor.selection.characterFormat.strikethrough !== 'None';
        this.tableTextProperties.isSuperscript = this.documentEditor.selection.characterFormat.baselineAlignment === 'Superscript';
        this.tableTextProperties.isSubscript = this.documentEditor.selection.characterFormat.baselineAlignment === 'Subscript';
        this.tableTextProperties.fontName = this.documentEditor.selection.characterFormat.fontFamily;
        this.tableTextProperties.fontSize = this.documentEditor.selection.characterFormat.fontSize;
        this.tableTextProperties.fontColor = this.documentEditor.selection.characterFormat.fontColor;
        (this.tableHighlightColor.element.firstElementChild as HTMLElement).style.backgroundColor = this.appliedHighlightColor;
        this.tableTextProperties.applyHighlightColorAsBackground(this.documentEditor.selection.characterFormat.highlightColor);
        this.tableParagraphProperties.alignLeft = this.documentEditor.selection.paragraphFormat.textAlignment === 'Left';
        this.tableParagraphProperties.alignRight = this.documentEditor.selection.paragraphFormat.textAlignment === 'Right';
        this.tableParagraphProperties.alignCenter = this.documentEditor.selection.paragraphFormat.textAlignment === 'Center';
        this.tableParagraphProperties.alignJustify = this.documentEditor.selection.paragraphFormat.textAlignment === 'Justify';
        if (this.documentEditor.editor) {
            this.tableProperties.canMerge = this.documentEditor.editor.canMergeCells()
        }
        this.tableProperties.topMargin = this.documentEditor.selection.cellFormat.topMargin || 0;
        this.tableProperties.leftMargin = this.documentEditor.selection.cellFormat.leftMargin || 0;
        this.tableProperties.bottomMargin = this.documentEditor.selection.cellFormat.bottomMargin || 0;
        this.tableProperties.rightMargin = this.documentEditor.selection.cellFormat.rightMargin || 0;
        let styleName: string = this.documentEditor.selection.paragraphFormat.styleName;
        if (styleName) {
            this.tableStyleDropDown.value = styleName;
        } else {
            this.styleDropDown.value = '';
        }
    }
    private showHFPropertiesPaneHelper(): void {
        this.headerFooterProperties.headerFromTop = this.documentEditor.selection.sectionFormat.headerDistance;
        this.headerFooterProperties.footerFromTop = this.documentEditor.selection.sectionFormat.footerDistance;
        this.headerFooterProperties.isFirstPage = this.documentEditor.selection.sectionFormat.differentFirstPage;
        this.headerFooterProperties.isOddEvenPage = this.documentEditor.selection.sectionFormat.differentOddAndEvenPages;
        document.getElementById("headerFromTop").addEventListener('keydown', this.headerFooterProperties.onHeaderValueChange);
        document.getElementById("footerFromTop").addEventListener('keydown', this.headerFooterProperties.onFooterValueChange);
        document.getElementById("headerFromTop").addEventListener('blur', this.headerFooterProperties.changeHeaderValue);
        document.getElementById("footerFromTop").addEventListener('blur', this.headerFooterProperties.changeFooterValue);
    }
    private showProperties = (property: string): void => {
        switch (property) {
            case 'text':
                this.isRetrieving = true;
                this.updateTextModel();
                this.isRetrieving = false;
                this.textProperties.show = true;
                this.enableDisablePropertyPaneButton(true);
                this.tableProperties.show = false;
                this.headerFooterProperties.show = false;
                this.imageProperties.show = false;
                this.tocProperties.showTocPane(false);
                this.insertFieldProperties.show = false;
                break;
            case 'table':
                this.isRetrieving = true;
                this.updateTableTextModel();
                this.isRetrieving = false;
                this.tableProperties.show = true;
                this.enableDisablePropertyPaneButton(true);
                this.textProperties.show = false;
                this.headerFooterProperties.show = false;
                this.imageProperties.show = false;
                this.tocProperties.showTocPane(false);
                this.insertFieldProperties.show = false;
                break;
            case 'image':
                this.imageProperties.show = true;
                this.imageProperties.wireEvents();
                this.imageProperties.updateImageProperties();
                this.enableDisablePropertyPaneButton(true);
                this.tableProperties.show = false;
                this.textProperties.show = false;
                this.headerFooterProperties.show = false;
                this.tocProperties.showTocPane(false);
                this.insertFieldProperties.show = false;
                break;
            case 'headerfooter':
                this.showHFPropertiesPaneHelper();
                this.headerFooterProperties.show = true;
                this.enableDisablePropertyPaneButton(false);
                this.imageProperties.show = false;
                this.tableProperties.show = false;
                this.textProperties.show = false;
                this.tocProperties.showTocPane(false);
                this.insertFieldProperties.show = false;
                break;
            case 'toc':
                this.tocProperties.showTocPane(true);
                this.enableDisablePropertyPaneButton(false);
                this.headerFooterProperties.show = false;
                this.imageProperties.show = false;
                this.tableProperties.show = false;
                this.textProperties.show = false;
                this.insertFieldProperties.show = false;
                break;
            case 'field':
                this.insertFieldProperties.show = true;
                // this.tocProperties.showTocPane(true);
                this.enableDisablePropertyPaneButton(false);
                this.tocProperties.show = false;
                this.headerFooterProperties.show = false;
                this.imageProperties.show = false;
                this.tableProperties.show = false;
                this.textProperties.show = false;
                break;
        }
    }
    public updateUndoRedoBtn = (): void => {
        let undo: HTMLElement = document.getElementById('undo');
        let redo: HTMLElement = document.getElementById('redo');
        if (!isNullOrUndefined(this.documentEditor.editorHistory) && this.documentEditor.editorHistory.canUndo()) {
            this.toolBarModule.enableItems(undo.parentElement, true);
        } else {
            this.toolBarModule.enableItems(undo.parentElement, false);
        }
        if (!isNullOrUndefined(this.documentEditor.editorHistory) && this.documentEditor.editorHistory.canRedo()) {
            this.toolBarModule.enableItems(redo.parentElement, true);
        } else {
            this.toolBarModule.enableItems(redo.parentElement, false);
        }
    }
    private onUseLocalClipboard = (): void => {
        if (this.documentEditor.enableLocalPaste && document.getElementById('useLocalClipboard').classList.contains('e-btn-toggle')) {
            document.getElementById('useLocalClipboard').className = 'e-tbar-btn e-tbtn-txt e-control e-btn';
            this.documentEditor.enableLocalPaste = false;
        } else {
            this.documentEditor.enableLocalPaste = true;
            document.getElementById('useLocalClipboard').className = 'e-tbar-btn e-tbtn-txt e-control e-btn e-btn-toggle';
        }
        this.documentEditor.focusIn();
    }
    public initializeFileTypeElements(): void {
        this.file = this.createFileTypeInput('uploadfileButton');
        this.file.addEventListener('change', this.onFileChange);
        this.file.setAttribute('accept', '.doc,.docx,.rtf,.txt,.sfdt');
        this.insertImage = this.createFileTypeInput('insertImageButton');
        this.insertImage.setAttribute('accept', '.jpeg,.jpg,.png,.gif,.bmp');
        this.insertImage.addEventListener('change', this.onInsertImage);
    }
    private createFileTypeInput(elemId: string): HTMLInputElement {
        let fileType: HTMLInputElement = createElement('input', {
            id: elemId,
            attrs: { 'type': 'file' }, styles: 'position:fixed; left:-100em'
        }) as HTMLInputElement;
        this.toolBarModule.element.parentElement.appendChild(fileType);
        return fileType;
    }
    private showCreateNewDocumentdialog = (): void => {
        if (document.getElementById('dialogContainer') === null || document.getElementById('dialogContainer') === undefined) {
            let container: HTMLElement = document.createElement('div');
            container.id = 'dialogContainer';
            container.className = 'e-de-ndlg-target';
            document.body.appendChild(container);
            this.newDialog.appendTo('#dialogContainer');
            container.parentElement.style.position = 'fixed';
            container.style.width = 'auto';
            container.style.height = 'auto';
            this.fileNameTextBox = document.getElementById('docText') as HTMLInputElement;
        }
        this.newDialog.show();
        this.newDialog.close = (): void => { this.documentEditor.focusIn(); };
        this.fileNameTextBox.value = '';
        this.fileNameTextBox.focus();
    }
    private templateSelection = (e: any): void => {
        let existing: any = document.getElementsByClassName('selection');
        if (existing.length > 0 && this.hasClass(existing[0], 'selection')) {
            let reg: any = new RegExp('(\\s|^)' + 'selection' + '(\\s|$)');
            existing[0].className = existing[0].className.replace(reg, ' ');
        }
        if (e !== null) {
            if (this.hasClass(e.target, 'template') && !this.hasClass(e.target.children[0], 'selection')) {
                e.target.children[0].className += ' ' + 'selection';
            } else if (this.hasClass(e.target, 'image') && !this.hasClass(e.target.parentElement, 'selection')) {
                e.target.parentElement.className += ' ' + 'selection';
            } else if (this.hasClass(e.target, 'innerTemplate') && !this.hasClass(e.target, 'selection')) {
                e.target.className += ' ' + 'selection';
            } else if (e.target.nodeName === 'LABEL' && !this.hasClass(e.target.previousSibling, 'selection')) {
                e.target.previousSibling.className += ' ' + 'selection';
            }
            existing = document.getElementsByClassName('selection');
        }
        (this.newDialog.element.querySelector('.e-footer-content').children[0] as HTMLElement).focus();
    }
    private hasClass = (el: HTMLElement, className: string): boolean => {
        return !!el.className.match(new RegExp('(\\s|^)' + className + '(\\s|$)'));
    }
    private initializeCreateNewDocumentDialog = (): void => {

          //this.httpClient.get<TemplateListItem[]>('http://localhost:52061/api/values/GetTemplateList')
          this.httpClient.get<Template[]>('http://localhost:52061/api/templates')
            .subscribe(data => { 
                

                this.cntnt += '<div id=\'mainContent\'>';
                this.cntnt += "<input type='text' class='e-input' id='docText' name='fileName' placeholder='Enter file name'>";
                this.cntnt += "<div id=templateDiv><div id='blanktemplate' class='template'><div id='Blank' class='innerTemplate'>";
                this.cntnt += "<label for='blank' class='labelDiv'>Blank</label></div>";

                // this.cntnt += '<input type=\'text\' class=\'e-input\' id=\'docText\' name=\'fileName\' placeholder=\'Enter file name\'>';
                // this.cntnt += '<div id=\'templateDiv\'>';

                data.forEach((item, index) => {
                    
                    if (index % 7 == 0)
                        this.cntnt += '<br/><br/>';

                    this.cntnt += '<div id=\'' + item.id + '\'  class=\'template\'>';
                    this.cntnt += '<div id=\'' + item.id + '\' class=\'innerTemplate\'>' +  item.templateName + '</div>';
                    //this.cntnt += '<label for=\'' + item.templateName + '\' class=\'labelDiv\'>' +  item.templateName + '</label>'
                    this.cntnt += '</div>';
                })
                this.cntnt += '</div></div>';

                this.newDialog = new Dialog({
                    header: 'New',
                    showCloseIcon: true,
                    closeOnEscape: true,
                    isModal: true,
                    beforeOpen: (): void => { this.documentEditor.focusIn(); },
                    buttons: [
                        {
                            'click': () => {
                                if (this.documentEditor.documentName === '') {
                                    this.documentEditor.documentName = 'Untitled';
                                }
                                this.loadDocumentTemplate();
                            },
                            buttonModel: {
                                content: 'Ok',
                                cssClass: 'e-de-template-btn-height',
                                isPrimary: true
                            }
                        },
                        {
                            'click': () => {
                                this.newDialog.hide();
                            },
                            buttonModel: {
                                cssClass: 'e-de-template-btn-height',
                                content: 'Cancel'
                            }
                        }
                    ],
                /* tslint:disable */
                content: this.cntnt,
                //    content: "<div id='mainContent'>" +
                //    "<input type='text' class='e-input' id='docText' name='fileName' placeholder='Enter file name'>" +
                //    "<div id=templateDiv><div id='blanktemplate' class='template'><div id='Blank' class='innerTemplate'>" +
                //    "1</div>" +
                //    "<label for='blank' class='labelDiv'>Blank</label></div>" +
                //    "<div id='notestemplate' class='template'><div id='Notes' class='innerTemplate'>" +
                //    "2</div>" +
                //    "<label for='notes' class='labelDiv'>Notes</label></div>" +
                //    "<div id='spectemplate' class='template'><div id='Spec Design' class='innerTemplate'>" +
                //    "3</div>" +
                //    "<label for='spec' class='labelDiv'>Spec design</label></div>" +
                //    "</div></div>",
            /* tslint:enable */
            });

          });

    }
    private loadDocumentTemplate = (): void => {
        let selectedItem: any = document.getElementsByClassName('selection');
        this.documentEditor.documentName = this.fileNameTextBox.value;
        if (selectedItem.length > 0) {
            let selectedItemId: string = selectedItem[0].id;
            if (selectedItemId === 'Blank') {
                this.documentEditor.openBlank(); //this.documentEditor.serialize())
                //console.log(this.documentEditor.serialize());
                this.titleBar.saveTemplate=true;
            } else {
                let urlStr = "";
                if(this.isTemplateEditing){
                    urlStr="http://localhost:52061/api/templates/";
                }
                else{
                    urlStr="http://localhost:52061/api/templates/document/";
                }
                this.httpClient.get<Template>(urlStr + selectedItemId)
                .subscribe(data => {
                    if(this.isTemplateEditing){
                        this.titleBar.saveTemplate=true;
                        this.titleBar.docTemplate.id = parseInt(selectedItemId);
                    }
                    
                    console.log(data);
                    // this.documentEditor.open(JSON.stringify(data.templateContent));
                    this.documentEditor.open(data.templateContent);
                });
            }
        } else {
            this.documentEditor.openBlank();
        }
        this.newDialog.hide();
    }

    private insertField = (): void => {
        this.showProperties('field');
    }
    //#endregion
}

export class TextPropertiesModel {
    public deComponent: EditorComponent;
    fontStyle: { [key: string]: Object }[] = [{ FontName: 'Algerian' }, { FontName: 'Arial' },
    { FontName: 'Calibri' }, { FontName: 'Cambria' }, { FontName: 'Cambria Math' }, { FontName: 'Candara' },
    { FontName: 'Courier New' }, { FontName: 'Georgia' }, { FontName: 'Impact' }, { FontName: 'Segoe Print' },
    { FontName: 'Segoe Script' }, { FontName: 'Segoe UI' }, { FontName: 'Symbol' },
    { FontName: 'Times New Roman' }, { FontName: 'Verdana' }, { FontName: 'Windings' }
    ];
    fontfields: Object = { text: 'FontName', value: 'FontName' };
    fontquery: Query = new Query().select(['FontName']);
    fontSizeSource: string[] = ['8', '9', '10', '11', '12', '14', '16', '18', '20', '22', '24', '26', '28', '36', '48', '72', '96'];
    buttonHeight: string = '32px';
    buttonWidth: string = '40.5px';
    show: boolean = true;
    bold: boolean = false;
    get isBold(): boolean {
        return this.bold;
    }
    @Input()
    set isBold(value: boolean) {
        if (this.bold !== value) {
            this.bold = value;
            this.triggerPropertyChange('bold', value);
        }
    }
    italic: boolean = false;
    get isItalic(): boolean {
        return this.italic;
    }
    @Input()
    set isItalic(value: boolean) {
        if (this.italic !== value) {
            this.italic = value;
            this.triggerPropertyChange('italic', value);
        }
    }
    underline: boolean = false;
    get isUnderline(): boolean {
        return this.underline;
    }
    @Input()
    set isUnderline(value: boolean) {
        if (this.underline !== value) {
            this.underline = value;
            this.triggerPropertyChange('underline', value);
        }
    }
    strikethrough: boolean = false;
    get isStrikethrough(): boolean {
        return this.strikethrough;
    }
    @Input()
    set isStrikethrough(value: boolean) {
        if (this.strikethrough !== value) {
            this.strikethrough = value;
            this.triggerPropertyChange('strikethrough', value);
        }
    }
    superscript: boolean = false;
    get isSuperscript(): boolean {
        return this.superscript;
    }
    @Input()
    set isSuperscript(value: boolean) {
        if (this.superscript !== value) {
            this.superscript = value;
            this.triggerPropertyChange('superscript', value);
        }
    }
    subscript: boolean = false;
    get isSubscript(): boolean {
        return this.subscript;
    }
    @Input()
    set isSubscript(value: boolean) {
        if (this.subscript !== value) {
            this.subscript = value;
            this.triggerPropertyChange('subscript', value);
        }
    }
    m_fontName: string = 'Calibri';
    get fontName(): string {
        return this.m_fontName;
    }
    @Input()
    set fontName(value: string) {
        if (this.m_fontName !== value) {
            this.m_fontName = value;
            this.triggerPropertyChange('fontName', value);
        }
    }
    m_fontSize: number = 0;
    get fontSize(): number {
        return this.m_fontSize;
    }
    @Input()
    set fontSize(value: number) {
        if (this.m_fontSize !== value) {
            this.m_fontSize = value;
            this.triggerPropertyChange('fontSize', value);
        }
    }
    m_fontColor: string = '#000000';
    get fontColor(): string {
        return this.m_fontColor;
    }
    set fontColor(value: string) {
        if (value && this.m_fontColor !== value) {
            this.m_fontColor = value;
            this.triggerPropertyChange('fontColor', value);
        }
    }
    m_highlightColor: string;
    get highlightColor(): string {
        return this.m_highlightColor;
    }
    set highlightColor(value: string) {
        if (this.m_highlightColor !== value) {
            this.m_highlightColor = value;
            this.triggerPropertyChange('highlightColor', value);
        }
    }
    constructor(component: EditorComponent) {
        this.deComponent = component;
    }
    public propertyChange: Function;
    public triggerPropertyChange(propertyName: string, propertyValue: Object): void {
        if (!isNullOrUndefined(this.propertyChange)) {
            this.propertyChange({ propertyName: propertyName, propertyValue: propertyValue });
        }
    }

    public toggleBold(): void {
        this.isBold = !this.isBold;
    }
    public toggleItalic(): void {
        this.isItalic = !this.isItalic;
    }
    public toggleUnderline(): void {
        this.isUnderline = !this.isUnderline;
    }
    public toggleStrikethrough(): void {
        this.isStrikethrough = !this.isStrikethrough;
    }
    public toggleSuperscript(): void {
        this.isSuperscript = !this.isSuperscript;
    }
    public toggleSubscript(): void {
        this.isSubscript = !this.isSubscript;
    }
    hgltSplitObjClick(): void {
        this.deComponent.appliedHighlightColor = (this.deComponent.highlightColor.element.firstElementChild as HTMLElement).style.backgroundColor;
        this.applyHighlightColor(this.deComponent.appliedHighlightColor);
    }
    applyHighlightColor(color: string) {
        this.deComponent.appliedHighlightColor = color;
        let hgltColor = this.deComponent.getHighLightColor(color);
        this.deComponent.documentEditor.selection.characterFormat.highlightColor = hgltColor;
    }
    closePopup() {
        document.getElementById('highlight_color_ppty').style.display = 'none';
    }
    openPopup() {
        document.getElementById('highlight_color_ppty').style.display = 'block';
    }
    onHighLightColor(event: MouseEvent) {
        this.applyHighlightColor((event.currentTarget as HTMLElement).style.backgroundColor);
        this.deComponent.highlightColor.toggle();
    }
    applyHighlightColorAsBackground(color: HighlightColor) {
        let highlightColorElement: HTMLElement = document.getElementById('highlight_color_ppty');
        this.removeSelectedColorDiv(highlightColorElement);
        if (color === 'NoColor') {
            highlightColorElement.querySelector('#noColorDiv').classList.add('e-color-selected');
        } else if (color === 'Yellow') {
            highlightColorElement.querySelector('#yellowDiv').classList.add('e-color-selected');
        } else if (color === 'BrightGreen') {
            highlightColorElement.querySelector('#brightGreenDiv').classList.add('e-color-selected');
        } else if (color === 'Turquoise') {
            highlightColorElement.querySelector('#turquoiseDiv').classList.add('e-color-selected');
        } else if (color === 'Pink') {
            highlightColorElement.querySelector('#hotPinkDiv').classList.add('e-color-selected');
        } else if (color === 'Red') {
            highlightColorElement.querySelector('#redDiv').classList.add('e-color-selected');
        } else if (color === 'DarkBlue') {
            highlightColorElement.querySelector('#darkBlueDiv').classList.add('e-color-selected');
        } else if (color === 'Teal') {
            highlightColorElement.querySelector('#tealDiv').classList.add('e-color-selected');
        } else if (color === 'Green') {
            highlightColorElement.querySelector('#greenDiv').classList.add('e-color-selected');
        } else if (color === 'Violet') {
            highlightColorElement.querySelector('#violetDiv').classList.add('e-color-selected');
        } else if (color === 'DarkRed') {
            highlightColorElement.querySelector('#darkRedDiv').classList.add('e-color-selected');
        } else if (color === 'DarkYellow') {
            highlightColorElement.querySelector('#darkYellowDiv').classList.add('e-color-selected');
        } else if (color === 'Gray50') {
            highlightColorElement.querySelector('#gray50Div').classList.add('e-color-selected');
        } else if (color === 'Gray25') {
            highlightColorElement.querySelector('#gray25Div').classList.add('e-color-selected');
        } else if (color === 'Black') {
            highlightColorElement.querySelector('#blackDiv').classList.add('e-color-selected');
        } else if (color === 'Blue') {
            highlightColorElement.querySelector('#blueDiv').classList.add('e-color-selected');
        }
    }
    removeSelectedColorDiv(highlightColorElement: HTMLElement) {
        highlightColorElement.querySelector('#noColorDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#yellowDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#brightGreenDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#turquoiseDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#hotPinkDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#redDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#darkBlueDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#tealDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#greenDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#violetDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#darkRedDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#darkYellowDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#gray50Div').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#gray25Div').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#blackDiv').classList.remove('e-color-selected');
    }
}

export class ParagraphPropertiesModel {
    private documentEditorComponent: EditorComponent;
    stylequery: Query = new Query().select(['StyleName', 'Style']);
    stylesData = [{ StyleName: 'Normal', Class: 'e-font-icons e-edit-font' }];
    stylefields = { text: 'StyleName', value: 'StyleName' };
    styleName: string = undefined;
    buttonHeight = '32px';
    buttonWidth = '40.5px';
    fontClass: string = 'e-de-prop-font-button';
    lineHeight = [
        { text: 'Single' },
        { text: '1.15' },
        { text: '1.5' },
        { text: 'Double' }
    ];
    m_alignLeft: boolean;
    get alignLeft(): boolean {
        return this.m_alignLeft;
    }
    set alignLeft(value: boolean) {
        if (this.m_alignLeft !== value) {
            this.m_alignLeft = value;
        }
    }
    m_alignRight: boolean;
    get alignRight(): boolean {
        return this.m_alignRight;
    }
    set alignRight(value: boolean) {
        if (this.m_alignRight !== value) {
            this.m_alignRight = value;
        }
    }
    m_alignCenter: boolean;
    get alignCenter(): boolean {
        return this.m_alignCenter;
    }
    set alignCenter(value: boolean) {
        if (this.m_alignCenter !== value) {
            this.m_alignCenter = value;
        }
    }

    m_alignJustify: boolean;
    get alignJustify(): boolean {
        return this.m_alignJustify;
    }
    set alignJustify(value: boolean) {
        if (this.m_alignJustify !== value) {
            this.m_alignJustify = value;
        }
    }
    constructor(component: EditorComponent) {
        this.documentEditorComponent = component;
    }
    public propertyChange: Function;
    public triggerPropertyChange(propertyName: string, propertyValue: Object): void {
        if (!isNullOrUndefined(this.propertyChange)) {
            this.propertyChange({ propertyName: propertyName, propertyValue: propertyValue });
        }
    }
    updateOptions(args: any): void {
        this.updateStyleNames();
        args.popup.element.getElementsByClassName('create-style-footer')[0].addEventListener('click', this.createStyle.bind(this));
    }
    updateStyleNames() {
        this.styleName = !isNullOrUndefined((this.documentEditorComponent.styleDropDown as any).itemData) ? (this.documentEditorComponent.styleDropDown as any).itemData.StyleName : undefined;
        this.stylesData = this.constructStyleDropItems(this.documentEditorComponent.documentEditor.getStyles('Paragraph'));
    }
    createStyle() {
        this.documentEditorComponent.documentEditor.showDialog('Styles')
    }
    constructStyleDropItems(styles: any) {
        let collection: any = [];
        for (let styleObj of styles) {
            let obj: any = {};
            obj.StyleName = (styleObj as any).name;
            obj.Style = this.parseStyle((styleObj as any).style as string);
            collection.push(obj);
        }
        return collection;
    }
    closeStyleValue(): void {
        if (!isNullOrUndefined(this.styleName)) {
            this.documentEditorComponent.styleDropDown.value = this.styleName;
            this.documentEditorComponent.styleDropDown.dataBind();
        }
    }
    selectStyleValue(args: any): void {
        if (!args.isInteracted) {
            return;
        }
        setTimeout(() => { this.applyStyleValue(args); }, 10);
    }
    applyStyleValue(args: any) {
        if (!this.documentEditorComponent.documentEditor.isReadOnly && this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.documentEditor.editor.applyStyle(args.itemData.StyleName);
        }
    }
    closeBullets() {
        document.getElementById('bullet_list').style.display = 'none';
    }
    openBullets() {
        document.getElementById('bullet_list').style.display = 'block';
    }
    closeNumbering() {
        document.getElementById('numbering_list').style.display = 'none';
    }
    openNumbering() {
        document.getElementById('numbering_list').style.display = 'block';
    }
    lineSpacingAction(args: any) {
        let text: string = args.item.text;
        switch (text) {
            case 'Single':
                this.documentEditorComponent.documentEditor.selection.paragraphFormat.lineSpacing = 1;
                break;
            case '1.15':
                this.documentEditorComponent.documentEditor.selection.paragraphFormat.lineSpacing = 1.15;
                break;
            case '1.5':
                this.documentEditorComponent.documentEditor.selection.paragraphFormat.lineSpacing = 1.5;
                break;
            case 'Double':
                this.documentEditorComponent.documentEditor.selection.paragraphFormat.lineSpacing = 2;
                break;
        }
        setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
    }
    bulletSplitObjClick() {
        this.applyLastAppliedBullet();
    }
    applyLastAppliedBullet() {
        switch (this.documentEditorComponent.appliedLastBullet) {
            case 'dot':
                this.dotClick();
                break;
            case 'square':
                this.squareClick();
                break;
            case 'circle':
                this.circleClick();
                break;
            case 'flower':
                this.flowerClick();
                break;
            case 'tick':
                this.tickClick();
                break;
            case 'arrow':
                this.arrowClick();
                break;
        }
    }
    numberedUpLetterClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'upLetter';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'UpLetter');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedLowRomanClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'lowRoman';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'LowRoman');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedUpRomanClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'upRoman';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'UpRoman');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedLowLetterClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'lowLetter';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'LowLetter');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedDotClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'arabic';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'Arabic');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedNoneClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.documentEditor.editor.clearList();
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    dotClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'dot';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf0b7', 'Symbol');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    noneClick() {

        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.documentEditor.editor.clearList();
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    squareClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'square';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf0a7' + '\u0020', 'Wingdings');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    flowerClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'flower';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf076', 'Wingdings');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    tickClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'tick';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf0fc', 'Wingdings');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    arrowClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'arrow';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf0d8', 'Wingdings');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    circleClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'circle';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf06f' + '\u0020', 'Symbol');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberingSplitObjClick() {
        this.applyLastAppliedNumbering();
    }
    applyLastAppliedNumbering() {
        switch (this.documentEditorComponent.appliedLastNumbering) {
            case 'arabic':
                this.numberedDotClick();
                break;
            case 'lowLetter':
                this.numberedLowLetterClick();
                break;
            case 'upLetter':
                this.numberedUpLetterClick();
                break;
            case 'lowRoman':
                this.numberedLowRomanClick();
                break;
            case 'upRoman':
                this.numberedUpRomanClick();
                break;
        }
    }
    parseStyle(style: string): string {
        let domStyle: any = {};
        let styleObj: any = JSON.parse(style);
        let textDecoration: string = '';
        if (!isNullOrUndefined(styleObj.characterFormat.baselineAlignment) && styleObj.characterFormat.baselineAlignment !== 'Normal') {
            let vAlign: string = '';
            switch (styleObj.characterFormat.baselineAlignment) {
                case 'Superscript':
                    vAlign = "super";
                    break;
                case 'Subscript':
                    vAlign = "sub";
                    break;
            }
            if (vAlign.length > 1) {
                domStyle["vertical-align"] = vAlign;
            }
        }
        if (!isNullOrUndefined(styleObj.characterFormat.underline) && styleObj.characterFormat.underline !== 'None') {
            textDecoration += "underline ";
        }
        if (!isNullOrUndefined(styleObj.characterFormat.strikethrough) && styleObj.characterFormat.strikethrough !== 'None') {
            textDecoration += "line-through ";
        }
        if (!isNullOrUndefined(styleObj.characterFormat.fontSize)) {
            domStyle["font-size"] = (styleObj.characterFormat.fontSize + 'px');
        }
        if (!isNullOrUndefined(styleObj.characterFormat.fontFamily)) {
            domStyle["font-family"] = styleObj.characterFormat.fontFamily;
        }
        if (!isNullOrUndefined(styleObj.characterFormat.bold) && styleObj.characterFormat.bold) {
            domStyle["font-weight"] = "bold";
        }
        if (!isNullOrUndefined(styleObj.characterFormat.italic) && styleObj.characterFormat.italic) {
            domStyle["font-style"] = "italic";
        }
        if (!isNullOrUndefined(styleObj.characterFormat.fontColor)) {
            domStyle["color"] = styleObj.characterFormat.fontColor;
        }
        if (textDecoration.length > 1) {
            domStyle["text-decoration"] = textDecoration;
        }
        return domStyle;
    }
}

export class ImagePropertiesModel {
    widthNumericBox: NumericTextBox;
    heightNumericBox: NumericTextBox;
    aspectRatioBtn: CheckBox;
    isMaintainAspectRatio: boolean = false;
    isWidthApply: boolean = false;
    isHeightApply: boolean = false;
    numericFormat: string = 'n0';
    numericDecimal: number = 2;
    numericMax: number = 23500;
    numericMin: number = 0;
    show: boolean = true;
    documentEditor: DocumentEditorComponent;

    wireEvents(): void {
        document.getElementById('container_widthInput').addEventListener('keydown', this.onImageWidth);
        document.getElementById('container_heightInput').addEventListener('keydown', this.onImageHeight);
        document.getElementById('container_widthInput').addEventListener('blur', (): void => { this.applyImageWidth(); this.isWidthApply = false; });
        document.getElementById('container_heightInput').addEventListener('blur', (): void => { this.applyImageHeight(); this.isHeightApply = false; });
    }

    onImageWidth = (e: KeyboardEventArgs): void => {
        if (e.keyCode === 13) {
            setTimeout((): void => { this.applyImageWidth(); this.isWidthApply = false; }, 30);
        }
    }
    onImageHeight = (e: KeyboardEventArgs): void => {
        if (e.keyCode === 13) {
            setTimeout((): void => { this.applyImageHeight(); this.isHeightApply = false; }, 30);
        }
    }
    applyImageWidth = (): void => {
        if (!this.isMaintainAspectRatio) {
            // tslint:disable-next-line:max-line-length
            let width: number = this.widthNumericBox.value;
            let height: number = this.heightNumericBox.value;
            if (width > this.widthNumericBox.max) {
                width = this.widthNumericBox.max;
            }
            if (height > this.heightNumericBox.max) {
                height = this.heightNumericBox.max;
            }
            if (!(width === null || height === null)) {
                this.documentEditor.selection.imageFormat.resize(width, height);
            }
        } else if (this.isMaintainAspectRatio) {
            // tslint:disable-next-line:max-line-length
            let width: number = this.widthNumericBox.value;
            if (width > this.widthNumericBox.max) {
                width = this.widthNumericBox.max;
            }
            let ratio: number = width / this.documentEditor.selection.imageFormat.width;
            let height: number = this.heightNumericBox.value * ratio;
            this.heightNumericBox.value = height;
            if (!(width === null || height === null)) {
                this.documentEditor.selection.imageFormat.resize(width, height);
            }
        }
    }
    applyImageHeight = (): void => {
        if (!this.isMaintainAspectRatio) {
            // tslint:disable-next-line:max-line-length
            let width: number = this.widthNumericBox.value;
            let height: number = this.heightNumericBox.value;
            if (!(width === null || height === null)) {
                this.documentEditor.selection.imageFormat.resize(width, height);
            }
        } else if (this.isMaintainAspectRatio) {
            // tslint:disable-next-line:max-line-length
            let height: number = this.heightNumericBox.value;
            let ratio: number = height / this.documentEditor.selection.imageFormat.height;
            let width: number = this.widthNumericBox.value * ratio;
            this.widthNumericBox.value = width;
            if (!(width === null || height === null)) {
                this.documentEditor.selection.imageFormat.resize(width, height);
            }
        }
    }
    onAspectRatioBtnClick = (): void => {
        if (this.isMaintainAspectRatio) {
            this.isMaintainAspectRatio = false;
        } else {
            this.isMaintainAspectRatio = true;
        }
    }

    public updateImageProperties(): void {
        this.widthNumericBox.value = this.documentEditor.selection.imageFormat.width;
        this.heightNumericBox.value = this.documentEditor.selection.imageFormat.height;
    }

    public propertyChange: Function;
    public triggerPropertyChange(propertyName: string, propertyValue: Object): void {
        if (!isNullOrUndefined(this.propertyChange)) {
            this.propertyChange({ propertyName: propertyName, propertyValue: propertyValue });
        }
    }
}
export class InsertFieldPropertiesModel {
    show: boolean = false;
    public fields: FieldCategory[];
    documentEditorComponent: EditorComponent;
    service: DocumentEditorService;

    constructor(component: EditorComponent) {
        this.documentEditorComponent = component;
    }

    public onClose(): void {
        this.show = false;
        this.documentEditorComponent.showPropertiesPaneOnSelection();
    }

    public loadFields() {
        this.documentEditorComponent.documentEditorService.getFields().subscribe(data => {
            console.log(data);
            this.fields = data;
        });
    }

    public onInsertField(value: Field): void {
       this.documentEditorComponent.documentEditor.editor.insertText(value.displayName, false);
    }

}
export class HeaderFooterPropertiesModel {
    show: boolean = false;
    numericFormat: string = 'n0';
    numericDecimal: number = 2;
    numericMax: number = 23500;
    numericMin: number = 0;
    headerFromTop: number = 36;
    footerFromTop: number = 36;
    isFirstPage: boolean = false;
    isOddEvenPage: boolean = false;
    public deComponent: EditorComponent;
    constructor(component: EditorComponent) {
        this.deComponent = component;
    }
    public onClose(): void {
        this.show = false;
        this.deComponent.documentEditor.selection.closeHeaderFooter();
    }
    public onHeaderValue(): void {
        this.headerFromTop = this.deComponent.headerFromTopNumericBox.value;
    }
    public onFooterValue(): void {
        this.footerFromTop = this.deComponent.footerFromTopNumericBox.value;
    }
    public onHeaderValueChange = (e: KeyboardEventArgs): void => {
        if (e.keyCode === 13) {
            setTimeout((): void => { this.changeHeaderValue(); }, 30);
        }
    }
    public onFooterValueChange = (e: KeyboardEventArgs): void => {
        if (e.keyCode === 13) {
            setTimeout((): void => { this.changeFooterValue(); }, 30);
        }
    }
    public changeHeaderValue = (): void => {
        this.deComponent.documentEditor.selection.sectionFormat.headerDistance = this.deComponent.headerFromTopNumericBox.value;
    }
    public changeFooterValue = (): void => {
        this.deComponent.documentEditor.selection.sectionFormat.footerDistance = this.deComponent.footerFromTopNumericBox.value;
    }
    public changeFirstPageOptions = (): void => {
        if (!this.deComponent.documentEditor.isReadOnly) {
            if (this.isFirstPage) {
                this.isFirstPage = false;
            } else {
                this.isFirstPage = true;
            }
            this.deComponent.documentEditor.selection.sectionFormat.differentFirstPage = this.isFirstPage;
            setTimeout(function () { this.deComponent.documentEditor.focusIn(); }, 10);
        }
    }
    public changeoddOrEvenOptions = (): void => {
        if (!this.deComponent.documentEditor.isReadOnly) {
            if (this.isOddEvenPage) {
                this.isOddEvenPage = false;
            } else {
                this.isOddEvenPage = true;
            }
            this.deComponent.documentEditor.selection.sectionFormat.differentOddAndEvenPages = this.isOddEvenPage;
            setTimeout(function () { this.deComponent.documentEditor.focusIn(); }, 10);
        }
    }
}
export class TablePropertiesModel {
    show: boolean = false;
    documentEditorComponent: EditorComponent;
    borderColor: string = '#000000';
    canMerge: boolean = true;
    isTopMarginApply: boolean;
    isBottomMarginApply: boolean;
    isLeftMarginApply: boolean;
    isRightMarginApply: boolean;
    borderSizeContent: string = '1.5px';
    decimalFormat: string = 'n0';
    decimalNumber: number = 2;
    textBoxValue: number = 0;
    header0: object = { text: 'Table' };
    header1: object = { text: 'Text' };
    animation: any = { previous: { effect: 'None' }, next: { effect: 'None' } };
    borderWidth: number = 1;
    m_fillColor: string = "#000000";
    topMargin: number = 0;
    bottomMargin: number = 0;
    leftMargin: number = 0;
    rightMargin: number = 0;
    get fillColor(): string {
        return this.m_fillColor;
    }
    set fillColor(value: string) {
        if (this.m_fillColor !== value) {
            this.m_fillColor = value;
        }
    }

    constructor(component: EditorComponent) {
        this.documentEditorComponent = component;
    }
    public changeBackgroundColor(args: ColorPickerEventArgs): void {
        if (!this.documentEditorComponent.documentEditor.isReadOnly) {
            this.documentEditorComponent.documentEditor.selection.cellFormat.background = args.currentValue.hex;
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 10);
        }
    }
    public changeBorderSize(args: ColorPickerEventArgs): void {
        setTimeout(() => { this.borderColor = args.currentValue.hex; this.documentEditorComponent.outlineBorder.element.focus(); }, 10);
    }
    public openBorderSizePopup(): void {
        this.updateBorderDivColor();
        document.getElementById('borderSizeTarget').style.display = "block";
    }
    public closeBorderSizePopup(): void {
        document.getElementById("borderSizeTarget").style.display = "none";
    }
    public updateBorderDivColor(): void {
        let borderSizeColorElement: HTMLCollectionOf<Element> = document.getElementsByClassName('e-de-border-width');
        for (let i: number = 0; i < borderSizeColorElement.length; i++) {
            (borderSizeColorElement[i] as HTMLElement).style.borderBottomColor = this.borderColor;
        }
    }
    public onBorderSizeChange(value: string): void {
        this.documentEditorComponent.tableBorderSize.content = value;
        setTimeout(() => { this.documentEditorComponent.outlineBorder.element.focus(); }, 10);
    }
    public onOutlineBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('OutsideBorders'));
    }
    public onAllBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('AllBorders'));
    }
    public onInsideBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('InsideBorders'));
    }
    public onLeftBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('LeftBorder'));
    }
    public onVerticalBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('InsideVerticalBorder'));
    }
    public onRightBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('RightBorder'));
    }
    public onTopBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('TopBorder'));
    }
    public onHorizontalBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('InsideHorizontalBorder'));
    }
    public onBottomBorder(): void {
        this.documentEditorComponent.documentEditor.editor.applyBorders(this.getBorder('BottomBorder'));
    }
    public getBorder(border: BorderType): BorderSettings {
        let borderSizeContent: HTMLElement | string = this.documentEditorComponent.tableBorderSize.content;
        let lineWidth: number = (borderSizeContent.indexOf('No Border') >= 0) ? 0 : parseInt(borderSizeContent, 0);
        let linestyle: LineStyle = (lineWidth === 0) ? 'Cleared' : 'Single';
        let borderSettings: BorderSettings = {
            type: border,
            borderColor: this.borderColor,
            lineWidth: lineWidth,
            borderStyle: linestyle
        };
        return borderSettings;
    }
    public onMergeCell(): void {
        this.documentEditorComponent.documentEditor.editor.mergeCells();
    }
    public onInsertRowAbove(): void {
        this.documentEditorComponent.documentEditor.editor.insertRow(true);
    }
    public onInsertRowBelow(): void {
        this.documentEditorComponent.documentEditor.editor.insertRow(false);
    }
    public onInsertColumnLeft(): void {
        this.documentEditorComponent.documentEditor.editor.insertColumn(true);
    }
    public onInsertColumnRight(): void {
        this.documentEditorComponent.documentEditor.editor.insertColumn(false);
    }
    public onDeleteRow(): void {
        this.documentEditorComponent.documentEditor.editor.deleteRow();
    }
    public onDeleteColumn(): void {
        this.documentEditorComponent.documentEditor.editor.deleteColumn();
    }
    public onTopMargin = (e: KeyboardEventArgs): void => {
        if (e.keyCode === 13) {
            setTimeout(() => { this.applyTopMargin(); this.isTopMarginApply = false; }, 30);
        }
    }
    public onBottomMargin = (e: KeyboardEventArgs): void => {
        if (e.keyCode === 13) {
            setTimeout(() => { this.applyBottomMargin(); this.isBottomMarginApply = false; }, 30);
        }
    }
    public onLeftMargin = (e: KeyboardEventArgs): void => {
        if (e.keyCode === 13) {
            setTimeout(() => { this.applyLeftMargin(); this.isLeftMarginApply = false; }, 30);
        }
    }
    public onRightMargin = (e: KeyboardEventArgs): void => {
        if (e.keyCode === 13) {
            setTimeout(() => { this.applyRightMargin(); this.isRightMarginApply = false; }, 30);
        }
    }
    public applyTopMargin = (): void => {
        if (!this.isTopMarginApply) {
            return;
        }
        this.documentEditorComponent.documentEditor.selection.cellFormat.topMargin = (this.documentEditorComponent.tableTopMargin.value > this.documentEditorComponent.tableTopMargin.max)
            ? this.documentEditorComponent.tableTopMargin.max : this.documentEditorComponent.tableTopMargin.value;
    }
    public applyBottomMargin = (): void => {
        if (!this.isBottomMarginApply) {
            return;
        }
        this.documentEditorComponent.documentEditor.selection.cellFormat.bottomMargin = (this.documentEditorComponent.tableBottomMargin.value > this.documentEditorComponent.tableBottomMargin.max)
            ? this.documentEditorComponent.tableBottomMargin.max : this.documentEditorComponent.tableBottomMargin.value;
    }
    public applyLeftMargin = (): void => {
        if (!this.isLeftMarginApply) {
            return;
        }
        this.documentEditorComponent.documentEditor.selection.cellFormat.leftMargin = (this.documentEditorComponent.tableLeftMargin.value > this.documentEditorComponent.tableLeftMargin.max)
            ? this.documentEditorComponent.tableLeftMargin.max : this.documentEditorComponent.tableLeftMargin.value;
    }
    public applyRightMargin = (): void => {
        if (!this.isRightMarginApply) {
            return;
        }
        this.documentEditorComponent.documentEditor.selection.cellFormat.rightMargin = (this.documentEditorComponent.tableRightMargin.value > this.documentEditorComponent.tableRightMargin.max)
            ? this.documentEditorComponent.tableRightMargin.max : this.documentEditorComponent.tableRightMargin.value;
    }
    public applyAlignTop(): void {
        this.documentEditorComponent.documentEditor.selection.cellFormat.verticalAlignment = 'Top';
    }
    public applyAlignBottom(): void {
        this.documentEditorComponent.documentEditor.selection.cellFormat.verticalAlignment = 'Bottom';
    }
    public applyAlignCenterHorizontal(): void {
        this.documentEditorComponent.documentEditor.selection.cellFormat.verticalAlignment = 'Center';
    }
    public onApplyTopMargin(): void {
        this.isTopMarginApply = true;
    }
    public onApplyBottomMargin(): void {
        this.isBottomMarginApply = true;
    }
    public onApplyLeftMargin(): void {
        this.isLeftMarginApply = true;
    }
    public onApplyRightMargin(): void {
        this.isRightMarginApply = true;
    }
    public onBorderSize5(): void {
        this.onBorderSizeChange('5px');
    }
    public onBorderSize4(): void {
        this.onBorderSizeChange('4px');
    }
    public onBorderSize3(): void {
        this.onBorderSizeChange('3px');
    }
    public onBorderSize2(): void {
        this.onBorderSizeChange('2px');
    }
    public onBorderSizePoint5(): void {
        this.onBorderSizeChange('1.5px');
    }
    public onBorderSizePoint1(): void {
        this.onBorderSizeChange('1');
    }
    public onNoBorder(): void {
        this.onBorderSizeChange('1');
    }
}
export class TocPropertiesModel {
    show: boolean = false;
    public documentEditor: DocumentEditorComponent;
    public element: HTMLElement;
    private elementId: string;
    private template1Div: HTMLElement;
    private contentStyle: DropDownList;
    private contentStyleLevel: DropDownList;
    private showPageNumber: CheckBox;
    private rightalignPageNumber: CheckBox;
    private hyperlink: CheckBox;
    private borderBtn: Button;
    private borderSize: DropDownButton;
    private borderStyle: DropDownButton;
    private updateBtn: Button;
    private cancelBtn: Button;
    private borderLevelStyle: DropDownList;
    public headerDiv: HTMLElement;
    private closeButton: HTMLElement;
    public docEditorComponent: EditorComponent;
    private prevContext: ContextType;
    constructor(docEditor: EditorComponent) {
        this.docEditorComponent = docEditor;
        this.elementId = 'container';
        // this.initializeTocPane();
    }

    public initializeTocPane = (): void => {
        // tslint:disable-next-line:max-line-length
        this.element = document.getElementById('toc_properties_pane');
        // this.element = 'padding:9px;width:269px'
        // this.element = createElement('div', { id: this.elementId + '_tocProperties', });
        this.tocHeaderDiv();
        this.initTemplates();
        this.tocOptionsDiv();
        this.contentStylesDropdown();
        this.checkboxContent();
        this.buttonDiv();
        this.wireEvents();
        this.updateTocProperties();
    }
    private updateTocProperties = (): void => {
        this.rightalignPageNumber.checked = true;
        this.showPageNumber.checked = true;
        this.hyperlink.checked = true;
    }
    private wireEvents = (): void => {
        this.cancelBtn.element.addEventListener('click', (): void => { this.onClose(); });
        this.updateBtn.element.addEventListener('click', this.onInsertToc);
        this.closeButton.addEventListener('click', (): void => { this.onClose(); });
    }
    private onClose = (): void => {
        if (this.docEditorComponent.showPropertiesPane
            && this.docEditorComponent.previousContext !== 'TableOfContents') {
            this.docEditorComponent.showPropertiesPaneOnSelection();
        } else {
            this.docEditorComponent.showPropertiesPane = false;
            this.showTocPane(false);
            this.docEditorComponent.enableDisablePropertyPaneButton(false);
            this.docEditorComponent.propertiesPane.show = false;
            setTimeout(() => { this.documentEditor.resize(), 30 });
        }
        setTimeout(() => { this.docEditorComponent.updateContainerSize(), 30 });
    }
    private tocHeaderDiv = (): void => {
        let headerDiv: HTMLElement = createElement('div', {
            id: this.elementId + 'toc_id',
            styles: 'display: block;'
        });
        this.element.appendChild(headerDiv);
        let title: HTMLElement = createElement('label', {
            className: 'e-de-prop-header-label'
        });
        title.textContent = 'Table of Contents';
        headerDiv.appendChild(title);
        this.closeButton = createElement('span', {
            className: 'e-de-icon-Close',
            styles: 'cursor: pointer;float:right;display:inline-block;color: #4A4A4A;'
        });
        headerDiv.appendChild(this.closeButton);
    }
    private initTemplates = (): void => {
        this.template1();
        // tslint:disable-next-line:max-line-length
        let div: HTMLElement = createElement('div', { styles: 'display:block;border-top: 1px solid #E0E0E0;margin-bottom:5px' }); this.element.appendChild(div);
    }

    private template1 = (): void => {
        this.template1Div = createElement('div', {
            className: 'toc-template1'
        });
        this.element.appendChild(this.template1Div);
        let templateContent1: HTMLElement = createElement('div', {
            className: 'toc-template1-content1'
        });
        templateContent1.textContent = 'HEADING______ 1';
        this.template1Div.appendChild(templateContent1);
        let templateContent2: HTMLElement = createElement('div', {
            className: 'toc-template1-content2'
        });
        templateContent2.textContent = 'HEADING______ 2';
        this.template1Div.appendChild(templateContent2);
        let templateContent3: HTMLElement = createElement('div', {
            className: 'toc-template1-content3'
        });
        templateContent3.textContent = 'HEADING______ 3';
        this.template1Div.appendChild(templateContent3);
    }

    private tocOptionsDiv = (): void => {
        let optionsDiv: HTMLElement = createElement('div', {
            className: 'toc-optionsDiv'
        });
        this.element.appendChild(optionsDiv);
        let label: HTMLElement = createElement('label', { className: 'e-de-prop-label' });
        label.textContent = 'Options';
        optionsDiv.appendChild(label);
    }

    private createDropdownOption(ulTag: HTMLElement, text: string): any {
        let liTag: HTMLElement = createElement('li', {
            styles: 'display:block',
            className: 'ui-wfloating-menuitem ui-wfloating-menuitem-md de-list-items  de-list-item-size'
        });
        ulTag.appendChild(liTag);
        let innerHTML: any;
        if (text === 'None') {
            innerHTML = '<div>' + text + '</div>';
        } else if (text === '1.5px') {
            // tslint:disable-next-line:max-line-length
            innerHTML = '<div>' + text + '<span class="ui-list-line" style="margin-left:10px;border-bottom-width:' + text + '"></span></div>';
        } else {
            // tslint:disable-next-line:max-line-length
            innerHTML = '<div>' + text + '<span class="ui-list-line" style="margin-left:20px;border-bottom-width:' + text + '"></span></div>';
        }
        let liInnerDiv: HTMLElement = createElement('div', {
            className: 'ui-list-header-presetmenu',
            id: 'ui-zlist0', innerHTML: innerHTML
        });
        liTag.appendChild(liInnerDiv);
        return liTag;
    }
    public createDropDownBtn(id: string, parentDiv: HTMLElement, iconCss: string, content: string[], selectedIndex: number): DropDownList {
        let buttonElement: HTMLButtonElement = createElement('input', { id: id }) as HTMLButtonElement;
        parentDiv.appendChild(buttonElement);
        // tslint:disable-next-line:max-line-length  
        let dropDownBtn: DropDownList = new DropDownList({ index: selectedIndex, dataSource: content, width: '75px', popupWidth: '75px', cssClass: 'e-de-prop-font-button' }, buttonElement);
        return dropDownBtn;
    }

    private contentStylesDropdown = (): void => {
        let contentStyleElement: HTMLElement = createElement('div', { id: 'contentstyle_div', styles: 'margin-bottom: 10px;' });
        contentStyleElement.setAttribute('title', 'Number of heading or outline levels to be shown in table of contents.');
        this.element.appendChild(contentStyleElement);
        // let items: ItemModel[] = [{ text: '___________', id: 'solid' }];

        // this.borderStyle = this.createDropDownBtn(
        //     this.elementId + '_borderStyleDiv',
        //     'width:120px;height:28px;margin-top:8px', contentStyleElement, 'e-de-icon-stroke-size', 'Solid', items
        // );
        let label: HTMLElement = createElement('label', { className: 'e-de-prop-sub-label', styles: 'margin-right:8px;' });
        label.textContent = 'Levels';
        contentStyleElement.appendChild(label);
        this.element.appendChild(contentStyleElement);
        let dataSource: string[] = ['1', '2', '3', '4', '5', '6', '7', '8', '9'];
        this.borderLevelStyle = this.createDropDownBtn(
            this.elementId + '_borderLevelDiv',
            contentStyleElement, '', dataSource, 2
        );
        this.borderLevelStyle.change = (args: any): void => {
            this.borderLevelStyle.value = args.item.value;
        };
        this.element.appendChild(contentStyleElement);
    }

    private checkboxContent = (): void => {
        let checkboxElement: HTMLElement = createElement('div', { id: 'toc_checkboxDiv', styles: 'margin-bottom:20px' });
        this.element.appendChild(checkboxElement);
        let showPageNumberDiv: HTMLElement = createElement('div', { className: 'e-de-prop-sub-label' });
        showPageNumberDiv.setAttribute('title', 'Show page numbers in table of contents.');
        checkboxElement.appendChild(showPageNumberDiv);
        // tslint:disable-next-line:max-line-length
        let showpagenumberCheckboxElement: HTMLElement = createElement('input', { id: 'showpagenumber', styles: 'width:12px;height:12px;margin-bottom:8px', className: 'e-de-prop-sub-label' });
        showPageNumberDiv.appendChild(showpagenumberCheckboxElement);
        this.showPageNumber = new CheckBox({
            label: 'Show page numbers',
        });
        this.showPageNumber.appendTo(showpagenumberCheckboxElement);
        let rightAlignDiv: HTMLElement = createElement('div', { className: 'e-de-prop-sub-label' });
        rightAlignDiv.setAttribute('title', 'Right align page numbers in table of contents.');
        checkboxElement.appendChild(rightAlignDiv);
        // tslint:disable-next-line:max-line-length
        let rightalignpagenumberCheckboxElement: HTMLElement = createElement('input', { id: 'rightalignpagenumber', styles: 'width:12px;height:12px', className: 'e-de-prop-sub-label' });
        rightAlignDiv.appendChild(rightalignpagenumberCheckboxElement);
        this.rightalignPageNumber = new CheckBox({
            label: 'Right align page numbers',
        });
        this.rightalignPageNumber.appendTo(rightalignpagenumberCheckboxElement);
        let hyperlinkDiv: HTMLElement = createElement('div', { className: 'e-de-prop-sub-label' });
        hyperlinkDiv.setAttribute('title', 'Use hyperlinks instead of page numbers.');
        checkboxElement.appendChild(hyperlinkDiv);
        // tslint:disable-next-line:max-line-length
        let hyperlinkCheckboxElement: HTMLElement = createElement('input', { id: 'hyperlinkdiv', styles: 'width:12px;height:12px', className: 'e-de-prop-sub-label' });
        hyperlinkDiv.appendChild(hyperlinkCheckboxElement);
        this.hyperlink = new CheckBox({
            label: 'Use hyperlinks',
        });
        this.hyperlink.appendTo(hyperlinkCheckboxElement);
    }

    private buttonDiv = (): void => {
        let footerElement: HTMLElement = createElement('div', { id: 'footerDiv', styles: 'float:right' });
        this.element.appendChild(footerElement);
        let updatebuttoncontentStyleElement: HTMLElement = createElement('button', { id: 'footerupdatebuttonDiv' });
        footerElement.appendChild(updatebuttoncontentStyleElement);
        this.updateBtn = new Button({
            content: 'Update', cssClass: 'btn-update', isPrimary: true
        });
        this.updateBtn.appendTo(updatebuttoncontentStyleElement);

        let cancelbuttoncontentStyleElement: HTMLElement = createElement('button', { id: 'footercancelbuttonDiv' });
        footerElement.appendChild(cancelbuttoncontentStyleElement);
        this.cancelBtn = new Button({
            content: 'Cancel', cssClass: 'btn-cancel'
        });
        this.cancelBtn.appendTo(cancelbuttoncontentStyleElement);
    }

    public showTocPane = (isShow: boolean, prevContextType?: ContextType): void => {

        if (!isShow && !this.show || (isShow && this.show)) {
            return;
        }
        this.show = isShow;
        this.updateBtn.content = this.documentEditor.selection.contextType === 'TableOfContents' ? 'Update' : 'Insert';
        this.prevContext = this.documentEditor.selection.contextType;
        this.documentEditor.resize();
        if (isShow) {
            this.updateBtn.element.focus();
        }
    }

    private onInsertToc = (): void => {
        // tslint:disable-next-line:max-line-length
        let tocSettings: TableOfContentsSettings = {
            startLevel: 1,
            endLevel: parseInt(this.borderLevelStyle.value as string, 0),
            includeHyperlink: this.hyperlink.checked,
            includePageNumber: this.showPageNumber.checked,
            rightAlign: this.rightalignPageNumber.checked
        };
        this.documentEditor.editor.insertTableOfContents(tocSettings);
        this.docEditorComponent.updateContainerSize();
    }
}
export class PropertiesPaneModel {
    isShow: boolean = false;
    get show(): boolean {
        return this.isShow;
    }
    @Input()
    set show(value: boolean) {
        if (this.isShow !== value) {
            this.isShow = value;
            this.triggerPropertyChange('propertypaneShow', value);
        }
    }
    public propertyChange: Function;
    public triggerPropertyChange(propertyName: string, propertyValue: Object): void {
        if (!isNullOrUndefined(this.propertyChange)) {
            this.propertyChange({ propertyName: propertyName, propertyValue: propertyValue });
        }
    }
}
export class TableTextPropertiesModel {
    public deComponent: EditorComponent;
    fontStyle: { [key: string]: Object }[] = [{ FontName: 'Algerian' }, { FontName: 'Arial' },
    { FontName: 'Calibri' }, { FontName: 'Cambria' }, { FontName: 'Cambria Math' }, { FontName: 'Candara' },
    { FontName: 'Courier New' }, { FontName: 'Georgia' }, { FontName: 'Impact' }, { FontName: 'Segoe Print' },
    { FontName: 'Segoe Script' }, { FontName: 'Segoe UI' }, { FontName: 'Symbol' },
    { FontName: 'Times New Roman' }, { FontName: 'Verdana' }, { FontName: 'Windings' }
    ];
    fontfields: Object = { text: 'FontName', value: 'FontName' };
    fontquery: Query = new Query().select(['FontName']);
    fontSizeSource: string[] = ['8', '9', '10', '11', '12', '14', '16', '18', '20', '22', '24', '26', '28', '36', '48', '72', '96'];
    buttonHeight: string = '32px';
    buttonWidth: string = '40.5px';
    show: boolean = true;
    bold: boolean = false;
    get isBold(): boolean {
        return this.bold;
    }
    @Input()
    set isBold(value: boolean) {
        if (this.bold !== value) {
            this.bold = value;
            this.triggerPropertyChange('bold', value);
        }
    }
    italic: boolean = false;
    get isItalic(): boolean {
        return this.italic;
    }
    @Input()
    set isItalic(value: boolean) {
        if (this.italic !== value) {
            this.italic = value;
            this.triggerPropertyChange('italic', value);
        }
    }
    underline: boolean = false;
    get isUnderline(): boolean {
        return this.underline;
    }
    @Input()
    set isUnderline(value: boolean) {
        if (this.underline !== value) {
            this.underline = value;
            this.triggerPropertyChange('underline', value);
        }
    }
    strikethrough: boolean = false;
    get isStrikethrough(): boolean {
        return this.strikethrough;
    }
    @Input()
    set isStrikethrough(value: boolean) {
        if (this.strikethrough !== value) {
            this.strikethrough = value;
            this.triggerPropertyChange('strikethrough', value);
        }
    }
    superscript: boolean = false;
    get isSuperscript(): boolean {
        return this.superscript;
    }
    @Input()
    set isSuperscript(value: boolean) {
        if (this.superscript !== value) {
            this.superscript = value;
            this.triggerPropertyChange('superscript', value);
        }
    }
    subscript: boolean = false;
    get isSubscript(): boolean {
        return this.subscript;
    }
    @Input()
    set isSubscript(value: boolean) {
        if (this.subscript !== value) {
            this.subscript = value;
            this.triggerPropertyChange('subscript', value);
        }
    }
    m_fontName: string = 'Calibri';
    get fontName(): string {
        return this.m_fontName;
    }
    @Input()
    set fontName(value: string) {
        if (this.m_fontName !== value) {
            this.m_fontName = value;
            this.triggerPropertyChange('fontName', value);
        }
    }
    m_fontSize: number = 0;
    get fontSize(): number {
        return this.m_fontSize;
    }
    @Input()
    set fontSize(value: number) {
        if (this.m_fontSize !== value) {
            this.m_fontSize = value;
            this.triggerPropertyChange('fontSize', value);
        }
    }
    m_fontColor: string = '#000000';
    get fontColor(): string {
        return this.m_fontColor;
    }
    set fontColor(value: string) {
        if (value && this.m_fontColor !== value) {
            this.m_fontColor = value;
            this.triggerPropertyChange('fontColor', value);
        }
    }
    m_highlightColor: string;
    get highlightColor(): string {
        return this.m_highlightColor;
    }
    set highlightColor(value: string) {
        if (this.m_highlightColor !== value) {
            this.m_highlightColor = value;
            this.triggerPropertyChange('highlightColor', value);
        }
    }
    constructor(component: EditorComponent) {
        this.deComponent = component;
    }
    public propertyChange: Function;
    public triggerPropertyChange(propertyName: string, propertyValue: Object): void {
        if (!isNullOrUndefined(this.propertyChange)) {
            this.propertyChange({ propertyName: propertyName, propertyValue: propertyValue });
        }
    }

    public toggleBold(): void {
        this.isBold = !this.isBold;
    }
    public toggleItalic(): void {
        this.isItalic = !this.isItalic;
    }
    public toggleUnderline(): void {
        this.isUnderline = !this.isUnderline;
    }
    public toggleStrikethrough(): void {
        this.isStrikethrough = !this.isStrikethrough;
    }
    public toggleSuperscript(): void {
        this.isSuperscript = !this.isSuperscript;
    }
    public toggleSubscript(): void {
        this.isSubscript = !this.isSubscript;
    }
    hgltSplitObjClick(): void {
        this.deComponent.appliedHighlightColor = (this.deComponent.tableHighlightColor.element.firstElementChild as HTMLElement).style.backgroundColor;
        this.applyHighlightColor(this.deComponent.appliedHighlightColor);
    }
    applyHighlightColor(color: string) {
        this.deComponent.appliedHighlightColor = color;
        let hgltColor = this.deComponent.getHighLightColor(color);
        this.deComponent.documentEditor.selection.characterFormat.highlightColor = hgltColor;
    }
    closePopup() {
        document.getElementById('table_highlight_color_ppty').style.display = 'none';
    }
    openPopup() {
        document.getElementById('table_highlight_color_ppty').style.display = 'block';
    }
    onHighLightColor(event: MouseEvent) {
        this.applyHighlightColor((event.currentTarget as HTMLElement).style.backgroundColor);
        this.deComponent.tableHighlightColor.toggle();
    }
    applyHighlightColorAsBackground(color: HighlightColor) {
        let highlightColorElement: HTMLElement = document.getElementById('table_highlight_color_ppty');
        this.removeSelectedColorDiv(highlightColorElement);
        if (color === 'NoColor') {
            highlightColorElement.querySelector('#table_noColorDiv').classList.add('e-color-selected');
        } else if (color === 'Yellow') {
            highlightColorElement.querySelector('#table_yellowDiv').classList.add('e-color-selected');
        } else if (color === 'BrightGreen') {
            highlightColorElement.querySelector('#table_brightGreenDiv').classList.add('e-color-selected');
        } else if (color === 'Turquoise') {
            highlightColorElement.querySelector('#table_turquoiseDiv').classList.add('e-color-selected');
        } else if (color === 'Pink') {
            highlightColorElement.querySelector('#table_hotPinkDiv').classList.add('e-color-selected');
        } else if (color === 'Red') {
            highlightColorElement.querySelector('#table_redDiv').classList.add('e-color-selected');
        } else if (color === 'DarkBlue') {
            highlightColorElement.querySelector('#table_darkBlueDiv').classList.add('e-color-selected');
        } else if (color === 'Teal') {
            highlightColorElement.querySelector('#table_tealDiv').classList.add('e-color-selected');
        } else if (color === 'Green') {
            highlightColorElement.querySelector('#table_greenDiv').classList.add('e-color-selected');
        } else if (color === 'Violet') {
            highlightColorElement.querySelector('#table_violetDiv').classList.add('e-color-selected');
        } else if (color === 'DarkRed') {
            highlightColorElement.querySelector('#table_darkRedDiv').classList.add('e-color-selected');
        } else if (color === 'DarkYellow') {
            highlightColorElement.querySelector('#table_darkYellowDiv').classList.add('e-color-selected');
        } else if (color === 'Gray50') {
            highlightColorElement.querySelector('#table_gray50Div').classList.add('e-color-selected');
        } else if (color === 'Gray25') {
            highlightColorElement.querySelector('#table_gray25Div').classList.add('e-color-selected');
        } else if (color === 'Black') {
            highlightColorElement.querySelector('#table_blackDiv').classList.add('e-color-selected');
        } else if (color === 'Blue') {
            highlightColorElement.querySelector('#table_blueDiv').classList.add('e-color-selected');
        }
    }
    removeSelectedColorDiv(highlightColorElement: HTMLElement) {
        highlightColorElement.querySelector('#table_noColorDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_yellowDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_brightGreenDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_turquoiseDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_hotPinkDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_redDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_darkBlueDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_tealDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_greenDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_violetDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_darkRedDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_darkYellowDiv').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_gray50Div').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_gray25Div').classList.remove('e-color-selected');
        highlightColorElement.querySelector('#table_blackDiv').classList.remove('e-color-selected');
    }
}
export class TableParagraphPropertiesModel {
    private documentEditorComponent: EditorComponent;
    stylequery: Query = new Query().select(['StyleName', 'Style']);
    stylesData = [{ StyleName: 'Normal', Class: 'e-font-icons e-edit-font' }];
    stylefields = { iconCss: 'Class', value: 'StyleName' };
    styleName: string = undefined;
    buttonHeight = '32px';
    buttonWidth = '40.5px';
    fontClass: string = 'e-de-prop-font-button';
    lineHeight = [
        { text: 'Single' },
        { text: '1.15' },
        { text: '1.5' },
        { text: 'Double' }
    ];
    m_alignLeft: boolean;
    get alignLeft(): boolean {
        return this.m_alignLeft;
    }
    set alignLeft(value: boolean) {
        if (this.m_alignLeft !== value) {
            this.m_alignLeft = value;
        }
    }
    m_alignRight: boolean;
    get alignRight(): boolean {
        return this.m_alignRight;
    }
    set alignRight(value: boolean) {
        if (this.m_alignRight !== value) {
            this.m_alignRight = value;
        }
    }
    m_alignCenter: boolean;
    get alignCenter(): boolean {
        return this.m_alignCenter;
    }
    set alignCenter(value: boolean) {
        if (this.m_alignCenter !== value) {
            this.m_alignCenter = value;
        }
    }

    m_alignJustify: boolean;
    get alignJustify(): boolean {
        return this.m_alignJustify;
    }
    set alignJustify(value: boolean) {
        if (this.m_alignJustify !== value) {
            this.m_alignJustify = value;
        }
    }
    constructor(component: EditorComponent) {
        this.documentEditorComponent = component;
    }
    public propertyChange: Function;
    public triggerPropertyChange(propertyName: string, propertyValue: Object): void {
        if (!isNullOrUndefined(this.propertyChange)) {
            this.propertyChange({ propertyName: propertyName, propertyValue: propertyValue });
        }
    }
    updateOptions(args: any): void {
        this.updateStyleNames();
        args.popup.element.getElementsByClassName('create-style-footer')[0].addEventListener('click', this.createStyle.bind(this));
    }
    updateStyleNames() {
        this.styleName = !isNullOrUndefined((this.documentEditorComponent.tableStyleDropDown as any).itemData) ? (this.documentEditorComponent.tableStyleDropDown as any).itemData.StyleName : undefined;
        this.stylesData = this.constructStyleDropItems(this.documentEditorComponent.documentEditor.getStyles('Paragraph'));
    }
    createStyle() {
        this.documentEditorComponent.documentEditor.showDialog('Styles')
    }
    constructStyleDropItems(styles: any) {
        let collection: any = [];
        for (let styleObj of styles) {
            let obj: any = {};
            obj.StyleName = (styleObj as any).name;
            obj.Style = this.parseStyle((styleObj as any).style as string);
            collection.push(obj);
        }
        return collection;
    }
    parseStyle(style: string): string {
        let domStyle: any = {};
        let styleObj: any = JSON.parse(style);
        let textDecoration: string = '';
        if (!isNullOrUndefined(styleObj.characterFormat.baselineAlignment) && styleObj.characterFormat.baselineAlignment !== 'Normal') {
            let vAlign: string = '';
            switch (styleObj.characterFormat.baselineAlignment) {
                case 'Superscript':
                    vAlign = "super";
                    break;
                case 'Subscript':
                    vAlign = "sub";
                    break;
            }
            if (vAlign.length > 1) {
                domStyle["vertical-align"] = vAlign;
            }
        }
        if (!isNullOrUndefined(styleObj.characterFormat.underline) && styleObj.characterFormat.underline !== 'None') {
            textDecoration += "underline ";
        }
        if (!isNullOrUndefined(styleObj.characterFormat.strikethrough) && styleObj.characterFormat.strikethrough !== 'None') {
            textDecoration += "line-through ";
        }
        if (!isNullOrUndefined(styleObj.characterFormat.fontSize)) {
            domStyle["font-size"] = (styleObj.characterFormat.fontSize + 'px');
        }
        if (!isNullOrUndefined(styleObj.characterFormat.fontFamily)) {
            domStyle["font-family"] = styleObj.characterFormat.fontFamily;
        }
        if (!isNullOrUndefined(styleObj.characterFormat.bold) && styleObj.characterFormat.bold) {
            domStyle["font-weight"] = "bold";
        }
        if (!isNullOrUndefined(styleObj.characterFormat.italic) && styleObj.characterFormat.italic) {
            domStyle["font-style"] = "italic";
        }
        if (!isNullOrUndefined(styleObj.characterFormat.fontColor)) {
            domStyle["color"] = styleObj.characterFormat.fontColor;
        }
        if (textDecoration.length > 1) {
            domStyle["text-decoration"] = textDecoration;
        }
        return domStyle;
    }
    closeStyleValue(): void {
        if (!isNullOrUndefined(this.styleName)) {
            this.documentEditorComponent.tableStyleDropDown.value = this.styleName;
            this.documentEditorComponent.tableStyleDropDown.dataBind();
        }
    }
    selectStyleValue(args: any): void {
        if (!args.isInteracted) {
            return;
        }
        setTimeout(() => { this.applyStyleValue(args); }, 10);
    }
    applyStyleValue(args: any) {
        if (!this.documentEditorComponent.documentEditor.isReadOnly && this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.documentEditor.editor.applyStyle(args.itemData.StyleName);
        }
    }
    closeBullets() {
        document.getElementById('table_bullet_list').style.display = 'none';
    }
    openBullets() {
        document.getElementById('table_bullet_list').style.display = 'block';
    }
    closeNumbering() {
        document.getElementById('table_numbering_list').style.display = 'none';
    }
    openNumbering() {
        document.getElementById('table_numbering_list').style.display = 'block';
    }
    lineSpacingAction(args: any) {
        let text: string = args.item.text;
        switch (text) {
            case 'Single':
                this.documentEditorComponent.documentEditor.selection.paragraphFormat.lineSpacing = 1;
                break;
            case '1.15':
                this.documentEditorComponent.documentEditor.selection.paragraphFormat.lineSpacing = 1.15;
                break;
            case '1.5':
                this.documentEditorComponent.documentEditor.selection.paragraphFormat.lineSpacing = 1.5;
                break;
            case 'Double':
                this.documentEditorComponent.documentEditor.selection.paragraphFormat.lineSpacing = 2;
                break;
        }
        setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
    }
    bulletSplitObjClick() {
        this.applyLastAppliedBullet();
    }
    applyLastAppliedBullet() {
        switch (this.documentEditorComponent.appliedLastBullet) {
            case 'dot':
                this.dotClick();
                break;
            case 'square':
                this.squareClick();
                break;
            case 'circle':
                this.circleClick();
                break;
            case 'flower':
                this.flowerClick();
                break;
            case 'tick':
                this.tickClick();
                break;
            case 'arrow':
                this.arrowClick();
                break;
        }
    }
    numberedUpLetterClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'upLetter';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'UpLetter');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedLowRomanClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'lowRoman';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'LowRoman');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedUpRomanClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'upRoman';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'UpRoman');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedLowLetterClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'lowLetter';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'LowLetter');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedDotClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastNumbering = 'arabic';
            this.documentEditorComponent.documentEditor.editor.applyNumbering('%1.', 'Arabic');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberedNoneClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.documentEditor.editor.clearList();
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    dotClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'dot';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf0b7', 'Symbol');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    noneClick() {

        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.documentEditor.editor.clearList();
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    squareClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'square';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf0a7' + '\u0020', 'Wingdings');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    flowerClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'flower';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf076', 'Wingdings');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    tickClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'tick';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf0fc', 'Wingdings');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    arrowClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'arrow';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf0d8', 'Wingdings');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    circleClick() {
        if (this.documentEditorComponent.documentEditor.editor) {
            this.documentEditorComponent.appliedLastBullet = 'circle';
            this.documentEditorComponent.documentEditor.editor.applyBullet('\uf06f' + '\u0020', 'Symbol');
            setTimeout(() => { this.documentEditorComponent.documentEditor.focusIn(); }, 30);
        }
    }
    numberingSplitObjClick() {
        this.applyLastAppliedNumbering();
    }
    applyLastAppliedNumbering() {
        switch (this.documentEditorComponent.appliedLastNumbering) {
            case 'arabic':
                this.numberedDotClick();
                break;
            case 'lowLetter':
                this.numberedLowLetterClick();
                break;
            case 'upLetter':
                this.numberedUpLetterClick();
                break;
            case 'lowRoman':
                this.numberedLowRomanClick();
                break;
            case 'upRoman':
                this.numberedUpRomanClick();
                break;
        }
    }
}
